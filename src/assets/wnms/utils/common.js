...
export function getUser () {
  // 获取当前用户下的护理单元
  const empInfo = window.localStorage.getItem("empInfo") || null;
  try {
    let parseInfo = JSON.parse(empInfo);

    if (parseInfo === null || parseInfo === "") {
      Message({
        typt: "error",
        message: "无法获取当前用户！"
      });
    ...
    return parseInfo.empId;
  } catch {
    Message({
      typt: "error",
      message: "json.parse转换失败"
    });
  }
}
...
export function getDeptInfo () {
  // 获取当前用户下的护理单元
  const deptInfo = getStorageUser("deptInfo");
...
export function getAllDeptList () {
  // 获取当前用户下的护理单元
  const list = getStorageUser("dataDeptInfoList");
...
export function newGetSelectData ({ data, params }) {
...
export function formatter (
  array,
  id,
  { value = "value", label = "label", flag = true }
) {
  if (flag) {
    for (let a of array) {
