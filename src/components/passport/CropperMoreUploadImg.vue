<template>
  <div class="passport-crop-img">
    ...
    />
    <div class="passport-uploader-img" v-for="(item, index) in list" :key="index"  style="margin-right: 20px">
      <template >
        <div @mouseenter="showThis(index)" @mouseleave="hidThis(index)" class="passport-up-img" :ref="'image'+index">
      ...
      <el-dialog width="580px" :visible.sync="dialogVisible" class="passport-look-imgDialog" >
        <img style="max-width: 60%;max-height: 60%" :src="imageUrl" alt="">
      </el-dialog>
    </div>
    <div v-show="list.length<3" class="passport-add-img" @click="handleOpenFile('')">
      上传图片
    </div>
  </div>
</template>
