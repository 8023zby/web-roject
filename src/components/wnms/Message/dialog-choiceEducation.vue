...
<template>
  ...
  >
    <div style="max-height: 600px;overflow-y: auto;">
      <el-tree
        id="showDialogTreeFormEducation"
...
<script>
    ...
    export default {
        ...
        methods: {
          init(Obj) {
            console.log('加载宣教选择弹窗')
            if (Obj) {
              this.SingleChoice = Obj.SingleChoice
            }
            let selectDept = JSON.parse(localStorage.getItem('orgInfo'))

            API.getEducationList({ nurseUnitId: Obj.nurseUnitId, orgId: selectDept.orgId }).then(response => {
              if (response.data) {
          ...
          chooseContent() {
            ...
            if (this.SingleChoice) {
              if (!this.treeCheckedObj.checked) {
                this.$message.error("未选择宣教！")
                return
              }
              arr = [{ eduid: this.treeCheckedObj.checked, eduname: this.treeCheckedObj.label }]
            } else {
