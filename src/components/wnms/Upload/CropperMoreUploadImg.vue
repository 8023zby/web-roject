<template>
  <div class="moreupload-crop-img">
      ...
      <div class="main-content">
        ...
        <div class="moreupload-tool-size-div">
          <!-- <div class="moreupload-tool-select">
            ...
            <div>
              <el-select @change="changeImgSize" v-model="imgSize">
    ...
    <div class="moreupload-uploader-img" v-for="(item, index) in list" :key="index" style="margin-right: 20px">
      <template>
        <div @mouseenter="showThis(index)" @mouseleave="hidThis(index)" class="moreupload-up-img" :ref="'image'+index">
          <img :src="item.imageUrl" class="showImg">
          <div class="showBtn" :ref="'czBtn'+index">
            <div>
              <p class="ImgBtn" @click.stop="showPic(item.imageUrl)">查看</p>
              <p class="ImgBtn" @click="handleOpenFile(index)">修改</p>
            </div>
          </div>
          <div class="closeBtn" :ref="'closeBtn'+index">
            <p title="删除" style="cursor: pointer;" class="el-icon-close" @click="delImage(index)"></p>
          </div>
          <div class="moreupload-wfsb" v-show="item.status===2">
      ...
      <el-dialog width="580px" :visible.sync="dialogVisible" class="moreupload-look-imgDialog" append-to-body>
        <img style="max-width: 60%;max-height: 60%" :src="imageUrl" alt="">
      </el-dialog>
    </div>
    <div v-show="list.length<3" class="moreupload-add-img" @click="handleOpenFile('')">
      上传图片
    </div>
  </div>
</template>
...
import { MsgShow, readFile } from '@/assets/wnms/utils/upload';
...
export default {
  ...
  methods: {
    ...
    async onChange (e) {
      ...
      const testmsg = /^(png|jpg)$/.test(type)
      */
      const types = [
        'image/png',
        'image/jpeg',
    ...
    configImage () {
      this.$refs.cropper.getCropBlob(data => {
        ...
        const isLt500K = data.size / 1024 / 1024 < 2;
        if (!isLt500K) {
          // 压缩
    ...
    uploadImg (file) {
      ...
      axios({
        ...
        if (res.data.status === 200) {
          ...
          if (this.imgIndex === '') {
            this.list.push({
              imageUrl: res.data.data.urlPath
            });
          } else {
