<template>
  ...
  <div class="temperature_sheet_print_container" id="temperature_sheet_print_container">
    ...
    <div>
      <div style="margin: auto; position: relative; width: 730px">
        <!-- svgIcon -->
        <svgIcon/>

        <!-- 体温单表格 -->
        <table class="tableSvg" style="width: 710px">
          <colgroup>
            <col width="20px">
            <col width="20px">
            <col width="39px">
          ...
          <tbody>
          <tr>
            <td class="every" colspan="3">日期</td>
            <td colspan="6" class="every" v-for="i in 7" :key="i + 'rq'">{{newCurWeekDate[i - 1] || ""}}</td>
          </tr>
          ...
          <tr class="height hour">
            ...
            <template v-for="i in 7">
              <template v-if="hours.length === 0">
                <td class="borderB" v-for="n in 6" :key="n + i + 'sj'" :class="{borderR : n === 6 && i !== 7}"></td>
              </template>
              <template v-else>
          ...
          <!-- 脉搏/体温 -->
          <tr class="height">
            <td colspan="2" rowspan="2" class="borderBT">
              次/分
            </td>
            <td rowspan="2" class="borderRB borderBT">°C</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 42 -->
          <tr class="height">
            <td colspan="2" rowspan="5" class="text-top borderBT">180</td>
            <td rowspan="5" class="borderRB text-top borderBT">42°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 41 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">160</td>
            <td rowspan="5" class="text-top borderRB borderBT">41°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 40 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">140</td>
            <td rowspan="5" class="text-top borderRB borderBT">40°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 39 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">120</td>
            <td rowspan="5" class="text-top borderRB borderBT">39°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 38 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">100</td>
            <td rowspan="5" class="text-top borderRB borderBT">38°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 37 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">80</td>
            <td rowspan="5" class="text-top borderRB borderBT">37°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <!-- 刻度 36 -->
          <tr class="height">
            <td rowspan="5" colspan="2" class="text-top borderBT">60</td>
            <td rowspan="5" class="text-top borderRB borderBT">36°</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
          ...
          <tr class="height">
            ...
            <td rowspan="7" class="borderRB" style="line-height: normal">
          ...
          <template v-for="(item, field) in signItemGroup">
            ...
            <template v-if="item.child.length > 1">
              ...
              <!-- 从第二行开始 -->
              <tr v-for="(childItem, index) in item.child" :key="childItem.signItemFieldName" v-if="index > 0">
                <td class="every" colspan="2">
                  {{item['child'][index].newSignItemShowName}}
                  {{item['child'][index].signItemUnit ? '('+item['child'][index].signItemUnit+')' : ''}}
                </td>
                <td :colspan="item['child'][index].colspan" v-for="n in item['child'][index].length">
            ...
            <tr v-else-if="item.child.length == 1">
              <td :class="{every: true, hx: field === 'huxi'}" colspan="3">
                {{item['child'][0].newSignItemShowName}}
                {{item['child'][0].signItemUnit ? '('+item['child'][0].signItemUnit+')' : ''}}
              </td>
              <template v-if="field === 'huxi'">
                <td :class="item['child'][0].data[n - 1] ? item['child'][0].data[n - 1].cls : ''" v-for="n in item['child'][0].length">
                  <template v-if="item['child'][0].data[n - 1]">
                    <span v-if="item['child'][0].data[n - 1].signValue==='呼吸机'"><img src="../../assets/nrms/images/rrr.svg"></span>
              ...
              <template v-else-if="field === 'xueya'">
                <template v-if="item['child'][0].data[n - 1]" v-for="n in item['child'][0].length">
                  <td :colspan="item['child'][0].colspan" :class="{every: true, text_scale: item['child'][0].data[n - 1].signValue.includes('下')}" >
                    <span>{{item['child'][0].data[n - 1].signValue}}</span>
                  </td>
                </template>
                <template v-else>
              ...
              <template v-else>
                <td :colspan="item['child'][0].colspan" class="every" v-for="n in item['child'][0].length">
                  <template v-if="item['child'][0].data[n - 1]">
          ...
          <tr>
            <td class="every" colspan="3">住院天数</td>
            <td colspan="6" class="every" v-for="i in 7" :key="i + 'z'">{{curWeekIn[i - 1] || ""}}</td>
          </tr>

          <tr>
            <td class="every" colspan="3">术后天数</td>
            <td colspan="6" class="every" v-for="i in 7" :key="i">{{curWeekDays.allSsDays[i - 1] || ""}}</td>
          </tr>

          <!-- 补空行 -->
          <tr v-for="n in (10 - showSignItemLength < 0 ? 0 : 10 - showSignItemLength)" :key="n">
            <td colspan="3" class="every">&nbsp;</td>
          ...
          <svg :width="svgWidth + 'px'" :height="svgHeight + 'px'">
            ...
            <template v-for="(item, index) in ttPoint">
              <a-tooltip :title="item.title" :key="'ttPoint-'+ index">
                <use xlink:href="#icon-tiwen-03" width="10" height="10" :x="item.x - 5" fill='red' :y="item.y - 5"></use>
              </a-tooltip>
            </template>
          ...
          <table>
            ...
            <tr>
              <td style="color: red">
                心<br/>率<br/>
                <svg width="10" height="10">
                  <use xlink:href="#icon-maibo-05"  fill='red'></use>
                </svg>
              </td>
            </tr>
            <tr>
              <td style="color: red">
                疼<br/>痛<br/>
                <svg width="10" height="10">
                  <use xlink:href="#icon-tiwen-03"  fill='red'></use>
                </svg>
              </td>
            </tr>
            <tr>
    ...
    parseRePoint (pointData, beginDate) {
      ...
      for (let i = 0; i < ttPoints.length; i++) {
        ...
        for (let j = 0; j < ttPoint.length; j++) {
          ...
          if (phValue) {
            ...
            // 绘制复测疼痛的点
            if (value !== phValue) {
              _this.fcttPoint.push({ x: xPx, y: wljwY, title: '复测疼痛:' + phValue, fill: '#fff', stroke: 'red' })
            }
          }
        }
      }
      ...
      for (let i = 0; i < coor.length; i++) {
        ...
        for (let j in json) {
          ...
          istt && _this.ttPoint.push({ type: 'tt', x: j, y: ttY, r: 4, stroke: 'red', title, fill: 'red' })
        }
      }
      // 根据心率和脉搏点，绘制多边形（脉搏短促）
      ...
      for (let i = 0; i < otherArray.length; i++) {
        ...
        if (+value > 180 && code === 'xinlv') {
          ...
          for (let k = 0; k < valueArray.length; k++) {
            _this.eventText.push({ x: xPx - 5.5, y: cy + height * k + 12, fill: '#000', text: valueArray[k] })
          }
        }
      }
    },
...
<style type="text/scss" lang="scss" scoped>
  ...
  .tableSvg {
    ...
    table-layout: fixed;
    font-size: 12px;
    tr:nth-child(3), tr:nth-child(4), tr:nth-child(6), tr:nth-child(11), tr:nth-child(16), tr:nth-child(21), tr:nth-child(26), tr:nth-child(31), tr:nth-child(36), tr:nth-child(41) {
      td:nth-child(1) {
        color: red;
      }
    }
