    ...
    <div style="position: relative">
      ...
      <table class="tableSvg">
        ...
        <tr>
          ...
          <td colspan="6" class="every" v-for="i in 7" :key="i">{{cur_week_date[i - 1] || ""}}</td>
        </tr>

        <tr>
          <td class="every" colspan="2">住院天数</td>
          <td colspan="6" class="every" v-for="i in 7"  :key="i">{{cur_week_in[i - 1] || ""}}</td>
        </tr>

        <tr>
          <td class="every" colspan="2">手术后天数</td>
          <td colspan="6" class="every" v-for="i in 7"  :key="i">{{cur_week_ss[i - 1] || ""}}</td>
        </tr>
        ...
        <tr class="height hour">
          ...
          <template v-for="i in 7">
            <template v-if="hours.length === 0">
              <td v-for="n in 6" :key="i + n" :class="{borderR : n === 6 && i !== 7}"></td>
            </template>
            <template v-else>
              <td v-for="(h, index) in hours" :class="{borderR : index === 5 && i !== 7}" :style="{color: h.temperatureItemColor}" :key="i +'' +h.temperatureConfigId">
                {{h.takeTemperatureTime}}
              </td>
            </template>
          </template>
        </tr>
        ...
        <!-- 循环可显示的体征项 -->
        <template v-for="item in show_sign_item">
          <tr :key="item.signId">
            <td :class="{every: true, hx: item.signItemFieldName==='huxi'}" colspan="2">
              {{item.signItemShowName}}{{item.signItemUnit ? '('+item.signItemUnit+')' : ''}}
            </td>
            <template v-if="item.signItemFieldName==='huxi'">
              <td :class="{'text-top' : n % 2 === 0, 'text-bottom': n % 2 !== 0}" :key="n" v-for="n in item.length">
            ...
            <template v-else-if="item.signItemFieldName === 'xueya'">
              <template v-if="item.data[n - 1]" v-for="n in item.length">
                <td :colspan="item.colspan" :key="n" :class="{every: true, text_scale: item.data[n - 1].itemValueType === '下肢'}" >
            ...
            <template v-else>
              <td :colspan="item.colspan" class="every" v-for="n in item.length"  :key="n">
                <template v-if="item.data[n - 1]">
        ...
        <!-- 补空行 -->
        <tr v-for="n in (10 - show_sign_item.length < 0 ? 0 : 10 - show_sign_item.length)"  :key="n">
      ...
      <div class="svg_container">
        <svg :width="svg_width + 'px'" :height="svg_height + 'px'">
          <!--复测体温，物理降温[虚线] -->
          <template v-for="(item, index) in wljw_line_data">
            <g style="fill: none; stroke: red; stroke-width: 2px" :key="'wljw_line_data-' + index" :title="item.title || '复测体温'">
              <path stroke-dasharray="3,3" :d="item"/>
            </g>
          </template>

          <!--复测疼痛 [虚线] -->
          <template v-for="(item, index) in ttfc_line_data">
            <g style="fill: none; stroke: red; stroke-width: 2px" :key="'ttfc_line_data-' + index" :title="item.title || '复测疼痛'">
              <path stroke-dasharray="3,3" :d="item"/>
            </g>
          </template>

          <!-- 体温，脉搏，心率，疼痛等[折线] -->
          <template v-for="(item, index) in broken_line">
            <path :stroke="item.stroke" stroke-width="2" fill='none' :key="index" :d="item.d" :title="item.title || '折线'" />
          </template>

          <!-- 脉搏短促[线][多变形] -->
          <template v-for="(item, index) in mbdc_line_polygon.line">
            <line :x1="item.x1" :y1="item.y1" :x2="item.x2" :y2="item.y2"  :key="index" stroke="red" stroke-width="1" :title="item.title || '多边形'" />
          </template>
          <template v-for="(item, index) in mbdc_line_polygon.polygon">
            <polygon :points="item.points" fill="none" :stroke="item.stroke" :key="index" stroke-width="2" :title="item.title || '多边形'"/>
          </template>

          <!-- 复测体温 物理降温[点  11] -->
          <template v-for="(item, index) in wljw_data">
            <a-tooltip :title="item.title" :key="'wljw_data-'+ index">
              <circle :cx="item.x" :cy="item.y" r="4" stroke-width="2" stroke="red" fill='#fff' :title="item.title || '复测体温点'"/>
            </a-tooltip>
          </template>

          <!-- 复测疼痛[点   ] -->
          <template v-for="(item, index) in ttfc_data">
            <a-tooltip :title="item.title" :key="'ttfc_data-'+ index">
          ...
          <!-- 体温，脉搏，心率，疼痛等[点   ] -->
          <template v-for="(item, index) in coincide_point">
            <a-tooltip :title="item.title" :key="'coincide_point-'+ index">
          ...
          <template v-for="(item, index) in intersect_point">
            <svg :key="'intersect_point-'+ index">
          ...
          <!-- 疼痛[点] -->
          <template v-for="(item, index) in tt_point">
            <a-tooltip :title="item.title" :key="'tt_point-'+ index">
          ...
          <!-- 事件 -->
          <template v-for="(item, index) in event_text">
            <svg v-if="item instanceof Array"  :key="index">
              <text v-for="(item2, index) in item"  :key="index" :x="item2.x" :y="item2.y" :fill="item2.fill" style="font-size:11px" :title="item.title || '事件'">{{item2.text}}</text>
            </svg>
            <text v-else :x="item.x" :y="item.y"  :key="index" :fill="item.fill" style="font-size:11px" :title="item.title || '事件'">{{item.text}}</text>
          </template>
...
import { getXY, getPolygon, getDateDiff, getHourIndex, lineX, splitGroup, segmentsIntr, getDoundary, getBlueLineFuncs, numToChinese } from '../../assets/nrms/js/TemperatureSheetUtils'
import moment from 'moment'
import { Tooltip } from 'ant-design-vue'

export default {
  name: '',
  props: ['patient', 'sheet_data', 'sign_item', 'cur_week_date', 'cur_week_in', 'cur_week_ss', 'hours'],
  data () {
    return {
      deptName: '',
      svg_width: 624,
      svg_height: 660,
      wljw_line_data: [], // 复测体温 物理降温虚线
      wljw_data: [], // 复测体温物理降温
      ttfc_line_data: [], // 复测疼痛 虚线
      ttfc_data: [], // 复测疼痛
      broken_line: [], // 体温、脉搏、心率、疼痛等折线
      coincide_point: [], // 体温、脉搏、心率、疼痛等点和重合的点
      intersect_point: [], // 体温、脉搏、心率、疼痛等 交叉的点
      event_text: [], // 事件文字
      tt_point: [], // 疼痛
      arrowhead: [], // 箭头
      mbdc_line_polygon: { line: [], polygon: [] }, // 脉搏短促[线][多边形]
      td: {
        width: 15,
        height: 14
      },
      ...
      /*
        point_line_data: {
          "tt": [{
            "dataTime": "2017-06-29 02:00:00", "date": "2017-06-29", phValue: 5,
            "hour": 2, "value": "9"
          }, {
            ...
            "dataTime": "2017-06-29 10:00:00", "date": "2017-06-29",
            "hour": 10, "value": "7"
          }],
          "xl": [{
  ...
  watch: {
    // 过滤后可以显示的体征项(底部)
    sheet_data: function (sheet_data) {
      this.reset()

      this.parseProps()
    },
    sign_item: function (sign_item) {
      this.reset()
  ...
  methods: {
      ...
      if (_type == 'update') {
        ...
        ApiAutoForm.getFormDictByType({ formType: this.formObj.formType }).then((res) => {
          ...
          if (res.data == null) {
            ...
            this.printSet = res.data.printSet ? res.data.printSet : '1' // 默认纵向打印
          }
        })
      }
    },
    reset () {
      this.show_sign_item = []
      this.point_line_data = []

      this.wljw_line_data = [] // 复测体温 物理降温虚线
      this.wljw_data = [] // 复测体温物理降温
      this.ttfc_line_data = [] // 复测疼痛 虚线
      this.ttfc_data = [] // 复测疼痛
      this.broken_line = [] // 体温、脉搏、心率、疼痛等折线
      this.coincide_point = [] // 体温、脉搏、心率、疼痛等点和重合的点
      this.intersect_point = [] // 体温、脉搏、心率、疼痛等 交叉的点
      this.event_text = [] // 事件文字
      this.tt_point = [] // 疼痛
      this.arrowhead = [] // 箭头
      this.mbdc_line_polygon = { line: [], polygon: [] } // 脉搏短促[线][多边形]
    },
    parseProps () {
      let sheet_data = this.sheet_data
      if (!this.sign_item || this.sign_item.length === 0 || !sheet_data) {
        return false
      }
      ...
      let data = { ...sheet_data }
      if (data['tiwen']) {
        for (let i = 0; i < data['tiwen'].length; i++) {
          let _d = data['tiwen'][i]
          if (!_d['signValue']) {
            continue
          }
          if (!/^[0-9]+.?[0-9]*$/.test(_d['signValue'])) {
            let t = data['tiwen'].splice(i, 1)
            data.eventData.push(...t)
            i--
          }
        }
      }
      ...
      // 心率大于180作为事件处理
      if (data['xinlv']) {
        for (let i = 0; i < data['xinlv'].length; i++) {
          let _d = data['xinlv'][i]
          if (!_d['signValue']) {
            continue
          }
          if (_d['signValue'] > 180) {
            let t = data['xinlv'].splice(i, 1)
            data.eventData.push(...t)
            i--
          }
        }
      ...
      for (let i = 0; i < this.sign_item.length; i++) {
        ...
        let data = sheet_data[_d['signItemFieldName']] || []
        if (
        // _d['signItemFieldName'] !== 'maibo'
          // && _d['signItemFieldName'] !== 'tiwen'
          // && _d['signItemFieldName'] !== 'huxi'
          // && _d['signItemFieldName'] !== 'tengtongdengji'
          // && _d['signItemFieldName'] !== 'shuzhangya'
          // && _d['signItemFieldName'] !== 'shousuoya'
          // && _d['signItemFieldName'] !== 'xinlv'
          // &&
          // 开启&&始终显示和有内容显示&&存储方式不能为空
          (
        // _d['isTemperatureDisplay']===true
        // &&
        ...
        if (_d['signItemFieldName'] === 'shousuoya') {
          _d['data'] = this.parseLineData(_d, data)
          console.log(2, _d['signItemFieldName'], _d.signStorageType, _d['data'])
          shousuoya = _d
        }
        // 舒张压
      ...
      // 合并收缩压|舒张压
      for (let i = 0; i < shousuoya.data.length; i++) {
        let ssy = shousuoya.data[i] ? shousuoya.data[i].signValue : ''
        let szy = shuzhangya.data[i] ? shuzhangya.data[i].signValue : ''
        let type = shuzhangya.data[i].itemValueType === '下肢' ? '(下)' : ''
        xueya_data.push({ signValue: ssy + (ssy ? '/' : '') + szy + type, itemValueType: shuzhangya.data[i].itemValueType })
      }
      // 必须设置半天存储
      items.unshift({
      ...
      items.unshift(huxi)
      this.show_sign_item = items

      this.init()
    },
    init () {
      this.parseRePoint(this.point_line_data, this.cur_week_date[0])
      this.parseEvent(this.point_line_data ? this.point_line_data.eventData : [], this.cur_week_date[0])
    },
    parseRePoint (pointData, beginDate) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (events[i].itemCode == 'tiwen' && (value === '拒测' || value === '外出' || value === '不升')) {
      ...
      let mbGroup = splitGroup([], mb)
      let xlGroup = splitGroup(xinlvLeaveEvents, xl)
      let wdGroup = splitGroup(tiwenLeaveEvents, wd)
      let ttGroup = splitGroup([], tt)
      // console.log(xinlvLeaveEvents, xlGroup);
      ...
      for (let i = 0; i < wdGroup.length; i++) {
        ...
        for (let j = 0; j < wdArray.length; j++) {
          ...
          let xy = getXY(_this.svg_height, _this.td, wdArray[j], beginDate, this.hours, 'wd')
          wdPoints[i].push(xy)
        }
      }
      // 心率
      ...
      for (let i = 0; i < xlGroup.length; i++) {
        ...
        for (let j = 0; j < xlArray.length; j++) {
          ...
          let xy = getXY(_this.svg_height, _this.td, xlArray[j], beginDate, this.hours, 'xl')
          xlPoints[i].push(xy)
        }
      }
      ...
      for (let i = 0; i < mbGroup.length; i++) {
        ...
        for (let j = 0; j < mbArray.length; j++) {
          ...
          let xy = getXY(_this.svg_height, _this.td, mbArray[j], beginDate, this.hours, 'mb')
          mbPoints[i].push(xy)
        }
        for (let j = 0; j < ttArray.length; j++) {
          if (!ttArray[j].signValue) continue
          let xy = getXY(_this.svg_height, _this.td, ttArray[j], beginDate, this.hours, 'tt')
          ttPoints[i].push(xy)
        }
      }
      ...
      for (let i = 0; i < wdPoints.length; i++) {
        ...
        for (let j = 0; j < wdPoint.length; j++) {
          ...
          if (phValue) {
            ...
            // 获取复测温度的坐标
            let wljwY = getXY(_this.svg_height, _this.td, wdPoint[j].point, beginDate, this.hours, 'wd', true).y
            _this.wljw_line_data.push('M' + xPx + ' ' + yPx + ' ' + xPx + ' ' + wljwY)
            // 绘制物理降温的点
            if (value !== phValue) {
              _this.wljw_data.push({ x: xPx, y: wljwY, title: '复测体温:' + phValue + '°C' })
            }
          }
        }
      }
      ...
      for (let i = 0; i < ttPoints.length; i++) {
        ...
        for (let j = 0; j < ttPoint.length; j++) {
          ...
          if (phValue) {
            ...
            // 获取复测疼痛的坐标
            let wljwY = getXY(_this.svg_height, _this.td, ttPoint[j].point, beginDate, this.hours, 'tt', true).y
            _this.ttfc_line_data.push('M' + xPx + ' ' + yPx + ' ' + xPx + ' ' + wljwY)
            // 绘制复测疼痛的点
            if (value !== phValue) {
              _this.ttfc_data.push({ x: xPx, y: wljwY, title: '复测疼痛:' + phValue })
            }
          }
        }
      }
      ...
      for (let i = 0; i < wdPoints.length; i++) {
        ...
        _this.broken_line.push({ stroke: 'blue', d: wdPath })
      }
      // 体温由于大于180分组了，所以点和线分开处理
      let _xlPoints = [[]]
      ...
      for (let i = 0; i < xlPoints.length; i++) {
        ...
        _this.broken_line.push({ stroke: 'red', d: xlPath })
      }
      ...
      for (let i = 0; i < mbPoints.length; i++) {
        ...
        _this.broken_line.push({ stroke: 'red', d: mbPath })
        _this.broken_line.push({ stroke: 'blue', d: ttPath })
      }
      ...
      for (let i = 0; i < coor.length; i++) {
        ...
        for (let j in json) {
          ...
          if (ismbxlwd && mbY === xlY && xlY === wdY && gw) { // 脉搏+心率+肛温[红圆+红圈+蓝圈]:两个圈在外围（红圈在最外层表示心率，篮圈在里层表示肛温）、一个圆在中间（红圆表示脉搏）
            ...
            _this.coincide_point.push({ type: '肛温', 'x': j, 'y': xlY, r: 7, stroke: 'blue', title, fill: '#fff' })
            _this.coincide_point.push({ type: 'mb', 'x': j, 'y': xlY, r: 3, stroke: 'red', title, fill: 'red' })
            ...
            _this.coincide_point.push({ type: 'mb', 'x': j, 'y': xlY, r: 7, stroke: 'red', title, fill: 'red' })
            _this.intersect_point.push({ type: '腋温', 'x': j, 'y': xlY, halfSize: 4, stroke: 'blue', title })
            ...
            _this.coincide_point.push({ type: 'mb', 'x': j, 'y': xlY, r: 7, stroke: 'red', title, fill: 'red' })
            _this.coincide_point.push({ type: '口温', 'x': j, 'y': xlY, r: 3, stroke: 'blue', title, fill: 'blue' })
          } else if (isxlwd && xlY === wdY && gw) { // 心率+肛温[红圈+蓝圈]
            let title = '心率:' + json[j].xl.value + '次，肛温:' + json[j].wd.value + '°C'
            if (xlY && wdY) {
              _this.coincide_point.push({ type: '肛温', 'x': j, 'y': xlY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': xlY, r: 3, stroke: 'blue', title, fill: '#fff' })
            }
            mbY && _this.coincide_point.push({
              type: 'mb',
              'x': j,
              'y': mbY,
              r: 4,
            ...
            if (xlY && wdY) {
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': xlY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.intersect_point.push({ type: '腋温', 'x': j, 'y': xlY, halfSize: 4, stroke: 'blue', title })
            }
            mbY && _this.coincide_point.push({
              type: 'mb',
            ...
            if (xlY && wdY) {
              _this.coincide_point.push({ type: 'xl', 'x': j, 'y': xlY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.coincide_point.push({ type: '口温', 'x': j, 'y': xlY, r: 3, stroke: 'blue', title, fill: 'blue' })
            }
            mbY && _this.coincide_point.push({
              type: 'mb',
            ...
            if (mbY && wdY) {
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': mbY, r: 7, stroke: 'blue', title, fill: '#fff' })
              _this.coincide_point.push({ type: '肛温', 'x': j, 'y': mbY, r: 3, stroke: 'red', title, fill: 'red' })
            }
            xlY && _this.coincide_point.push({
              type: 'xl',
              'x': j,
              'y': xlY,
              r: 4,
            ...
            if (mbY && wdY) {
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': mbY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.intersect_point.push({ type: '腋温', 'x': j, 'y': mbY, halfSize: 4, stroke: 'blue', title })
            }
            xlY && _this.coincide_point.push({
              type: 'xl',
              'x': j,
            ...
            if (mbY && wdY) {
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': mbY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.coincide_point.push({ type: '口温', 'x': j, 'y': mbY, r: 3, stroke: 'blue', title, fill: 'blue' })
            }
            xlY && _this.coincide_point.push({
              type: 'xl',
              'x': j,
            ...
            mbY && xlY && _this.coincide_point.push({
              type: 'mb',
              'x': j,
              'y': mbY,
              r: 4,
          ...
          if (ismbwd && mbY === fcwdY && fcgw) { // 脉搏+复测肛温[红圆+蓝圈]
            ...
            if (mbY && fcwdY) {
              _this.coincide_point.push({ type: 'mb', 'x': j, 'y': mbY, r: 7, stroke: 'red', title, fill: '#fff' })
              _this.intersect_point.push({ type: 'yw', 'x': j, 'y': mbY, halfSize: 4, stroke: 'blue', title })
            }
          ...
          function drawWd (point, j, wdY, t) {
            if (wdY && wdY < wdMax) {
              ...
              // console.log('2 -- '+ t + ' - ', wdY, point.wd.type, point.wd.value, point)
              if (point.wd.type === '腋温') {
                wdY && _this.intersect_point.push({
                  type: '腋温',
                  'x': j,
                  'y': wdY,
                  halfSize: 4,
          ...
          istt && _this.tt_point.push({ type: 'tt', x: j, y: ttY, r: 4, stroke: 'blue', title, fill: 'blue' })
        }
      }
      // 根据心率和脉搏点，绘制多边形（脉搏短促）
      // 统计出现的多边形
      let polygonArray = getPolygon(mbPoints, _xlPoints)
      // 绘制多边形
    ...
    parseBlueLine (polygonArray) {
      ...
      for (let j = 0; j < totalPoints.length; j++) {
        ...
        if (points.length === 2) {
          _this.mbdc_line_polygon.line.push({
            x1: points[0].x,
            y1: points[0].y,
            x2: points[1].x,
            y2: points[1].y,
      ...
      _this.mbdc_line_polygon.polygon.push(htmlde)
    },
    ...
    parseEvent (evenArray, beginDate) {
      ...
      let _this = this
      let svgW = _this.svg_width
      let svgH = _this.svg_height
      let tdW = _this.td.width // 获取td的宽度
      let daysW = _this.td.width * 6 // 一天的显示的宽度
      ...
      for (let i = 0; i < evenArray.length; i++) {
        ...
        evensXpx[dataTime] = getDateDiff(beginDate, evenArray[i].recordDate.split(' ')[0]) * daysW + tdW * getHourIndex(evenArray[i].itemHour, this.hours || []) - tdW / 2
      ...
      for (let i = 0; i < showArray.length; i++) {
        ...
        _this.event_text.push(svg_text)
      }
      // 绘制其他事件
      for (let i = 0; i < otherArray.length; i++) {
        let value = otherArray[i].signValue
        let time = otherArray[i].recordDate.split(' ')[0] + ' ' + otherArray[i].itemHour + ':00'
        // let xPx = getDateDiff(beginDate, time) * daysW;
        let xPx = getDateDiff(beginDate, otherArray[i].recordDate.split(' ')[0]) * daysW + tdW * getHourIndex(otherArray[i].itemHour, this.hours || []) - tdW / 2
        // if (value === "开呼吸机") {
        //   //绘制向上的箭头
        //   let x = xPx;
        //   let y1 = 42 * height;
        //   let y2 = 42 * height - 3 * height + 3;
        //   _this.arrowhead.push({ x1: x, y1: y1, x2: x, y2: y2, color: 'blue', title: '' });
        //   //在箭头旁边绘制呼吸机三字
        //   let textArray = ["呼", "吸", "机"];
        //   for (let j = 0; j < textArray.length; j++) {
        //     _this.event_text.push({ x: xPx - 15, y: 40 * height + j * 14 - 2, fill: "blue", text: textArray[j] });
        //   }
        // } else if (value === "关呼吸机") {
        //   //绘制向下的箭头
        //   let x = xPx;
        //   let y1 = 42 * height - 3 * height;
        //   let y2 = 42 * height - 3;
        //   _this.arrowhead.push({ x1: x, y1: y1, x2: x, y2: y2, color: 'blue', title: '' });
        // } else
        if (+value > 180) {
          ...
          let textArray = ['过', '快']
          for (let j = 0; j < textArray.length; j++) {
            _this.event_text.push({ x: xPx - 5.5, y: 3 * 14 + j * 14 - 2, fill: '#000', text: textArray[j] })
          }
        } else if (value === '不升') {
          ...
          for (let k = 0; k < valueArray.length; k++) {
            _this.event_text.push({ x: xPx - 5.5, y: cy + height * k + 12, fill: 'red', text: valueArray[k] })
          }
        } else if (value === '拒测' || value === '外出') {
          let valueArray = value.split('')
          // 显示在35度下面
          for (let k = 0; k < valueArray.length; k++) {
            _this.event_text.push({ x: xPx - 5.5, y: 3 * 14 + k * 14 - 2, fill: '#000', text: valueArray[k] })
          }
        }
      }
    },
    ...
    parseLineData (item, data) {
      let d = []
      for (let i = 0; i < this.cur_week_date.length; i++) {
        let date = this.cur_week_date[i] // 日期
        // 时间点
        if (item.signStorageType === 2) {
          for (let x = 0; x < this.hours.length; x++) {
            ...
            for (let j = 0; j < data.length; j++) {
              ...
              if (date === recordDate && h.takeTemperatureTime === +recordHour) {
                value = data[j]
              }
            }
            d.push(value)
