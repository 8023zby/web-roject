  ...
  <div class="temperature_sheet_print_container" id="temperature_sheet_print_container">
    ...
    <div class="formTable">
      <div class="table-head">
        <!-- 体温单表格日期时间 -->
        <table class="tableSvg" style="width: 750px;margin: 0 auto;">
          <colgroup>
            <col width="20px">
            <col width="20px">
            <col width="39px">
          ...
          <thead>
          <tr>
            <td class="every" colspan="4">日期</td>
        ...
        <div style="position: relative; margin: 0 auto;width: 750px;">
          ...
          <table class="tableSvg" style="width: 750px;">
            ...
            <tr class="height">
              ...
              <td rowspan="2" style="border-right-color: transparent;">°F</td>
              <td rowspan="2" class="borderRB">°C</td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
            ...
            <!-- 刻度 42 -->
            <tr class="height">
              <td colspan="2" rowspan="5" class="text-top">180</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                107.6°
              </td>
            ...
            <!-- 刻度 41 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">160</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                105.8°
              </td>
            ...
            <!-- 刻度 40 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">140</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                104°
              </td>
            ...
            <!-- 刻度 39 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">120</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                102.2°
              </td>
            ...
            <!-- 刻度 38 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">100</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                100.4°
              </td>
            ...
            <!-- 刻度 37 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">80</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                90°
              </td>
            ...
            <!-- 刻度 36 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">60</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                96.6°
              </td>
            ...
            <!-- 刻度 35 -->
            <tr class="height">
              <td rowspan="5" colspan="2" class="text-top">40</td>
              <td rowspan="5" class="text-top" style="border-right-color: transparent; text-align: left">
                95°
              </td>
            ...
            <!-- 疼痛等级 -->
            <tr class="height">
              <td rowspan="7" colspan="2" class="tt">
                <span>疼</span><br/><span>痛</span><br/><span>等</span><br/><span>级</span>
              </td>
              <td rowspan="7" colspan="2" class="borderRB" style="line-height: normal">
                <div
                  style="height: 100%; width: 100%; display: flex; justify-content: space-between; flex-direction: column; align-items: flex-end;">
            ...
            <!-- 循环可显示的体征项 -->
            <template v-for="(item, field) in signItemGroup">
              <!-- 有多个子项 -->
              <template v-if="item.child.length > 1">
                <tr :key="field">
                  <td :rowspan="item.child.length">{{item.name}}</td>
                  <td class="every" colspan="3">
                    {{item['child'][0].newSignItemShowName}}
                    {{item['child'][0].signItemUnit ? '('+item['child'][0].signItemUnit+')' : ''}}
                  </td>
                  <td :colspan="item['child'][0].colspan" v-for="n in item['child'][0].length">
                    <template v-if="item['child'][0].data[n - 1]">
                      <span>{{item['child'][0].data[n - 1].signValue}}</span>
                    </template>
                    <template v-else>
                      <span></span>
                    </template>
                ...
                <!-- 从第二行开始 -->
                <tr v-for="(childItem, index) in item.child" :key="childItem.signItemFieldName"
                    v-if="index > 0">
                  <td class="every" colspan="3">
                    {{item['child'][index].newSignItemShowName}}
                    {{item['child'][index].signItemUnit ? '('+item['child'][index].signItemUnit+')'
                    : ''}}
                  </td>
                  <td :colspan="item['child'][index].colspan"
                      v-for="n in item['child'][index].length">
                    <template v-if="item['child'][index].data[n - 1]">
                      <span>{{item['child'][index].data[n - 1].signValue}}</span>
                    </template>
                    <template v-else>
                      <span></span>
                    </template>
              ...
              <!-- 呼吸和血压当作没有子级来处理 -->
              <tr v-else-if="item.child.length == 1">
                <td :class="{every: true, hx: field === 'huxi'}" colspan="4">
                  {{item['child'][0].newSignItemShowName}}
                  {{item['child'][0].signItemUnit ? '('+item['child'][0].signItemUnit+')' : ''}}
                </td>
                <template v-if="field === 'huxi'">
                  <td :class="item['child'][0].data[n - 1] ? item['child'][0].data[n - 1].cls : ''"
                      v-for="n in item['child'][0].length">
                    <template v-if="item['child'][0].data[n - 1]">
                                            <span v-if="item['child'][0].data[n - 1].signValue==='呼吸机'"><img
                                              src="../../assets/nrms/images/rrr.svg"></span>
                      <span v-else>{{item['child'][0].data[n - 1].signValue}}</span>
                    </template>
                    <template v-else>
                      <span></span>
                    </template>
                ...
                <template v-else-if="field === 'xueya'">
                  <template v-if="item['child'][0].data[n - 1]" v-for="n in item['child'][0].length">
                    <td :colspan="item['child'][0].colspan"
                        :class="{every: true, text_scale: item['child'][0].data[n - 1].signValue.includes('下')}">
                      <span>{{item['child'][0].data[n - 1].signValue}}</span>
                    </td>
                  </template>
                  <template v-else>
                    <td :colspan="item['child'][0].colspan" class="every"><span></span></td>
                  </template>
                </template>
                <template v-else>
                  <td :colspan="item['child'][0].colspan" class="every"
                      v-for="n in item['child'][0].length">
                    <template v-if="item['child'][0].data[n - 1]">
                    <span v-if="item['child'][0].data[n - 1].signValue==='人工肛门'">
                      <img src="../../assets/nrms/images/five-star.svg">
                    </span>
                      <span v-else-if="item['child'][0].data[n - 1].signValue==='大便失禁'">
            ...
            <tr>
              <td class="every" colspan="4">术后天数</td>
              <td colspan="6" class="every" v-for="i in 7">{{curWeekDays.allSsDays[i - 1] || ''}}</td>
            </tr>

            <tr>
              <td class="every" colspan="4">住院天数</td>
              <td colspan="6" class="every" v-for="i in 7">{{curWeekIn[i - 1] || ''}}</td>
            </tr>

            <!-- 补空行 -->
            <tr v-for="n in (10 - showSignItemLength < 0 ? 0 : 10 - showSignItemLength)">
          ...
          <div class="svg_container">
            <svg :width="svgWidth + 'px'" :height="svgHeight + 'px'">
              ...
              <template v-for="(item, index) in fctwLine">
                <g style="fill: none; stroke: red; stroke-width: 1px" :key="'fctwLine-' + index"
                   :title="item.title || '复测体温'">
              ...
              <template v-for="(item, index) in fcttLine">
                <g style="fill: none; stroke: red; stroke-width: 1px" :key="'fcttLine-' + index"
                   :title="item.title || '复测疼痛'">
                  <path stroke-dasharray="3,3" :d="item"/>
                </g>
              </template>
              ...
              <template v-for="(item, index) in brokenLine">
                <path :key="'brokenLine' + index" :stroke="item.stroke" stroke-width="1" fill='none'
                      :d="item.d" :title="item.title || '折线'"/>
              </template>

              <!-- 脉搏短促[线][多变形] -->
              <template v-for="(item, index) in mbdcLinePolygon.line">
                <line :key="'mbdcLinePolygon.line' + index" :x1="item.x1" :y1="item.y1" :x2="item.x2"
                      :y2="item.y2" stroke="red" stroke-width="1" :title="item.title || '多边形'"/>
              </template>
              <template v-for="(item, index) in mbdcLinePolygon.polygon">
                <polygon :key="'mbdcLinePolygon.polygon' + index" :points="item.points" fill="none"
                         :stroke="item.stroke" stroke-width="1" :title="item.title || '多边形'"/>
              </template>

              <!-- 复测体温 物理降温[点  11] -->
              <template v-for="(item, index) in fctwPoint">
                <a-tooltip :title="item.title" :key="'fctwPoint-'+ index">
                  <use xlink:href="#icon-maibo-05" width="10" height="10" :x="item.x - 5" fill='red'
                       :y="item.y - 5"></use>
                </a-tooltip>
              </template>
              ...
              <template v-for="(item, index) in fswdPoint">
                <a-tooltip :title="item.title" :key="'fswdPoint-'+ index">
                  <use xlink:href="#icon-btn_zk" width="10" height="10" :x="item.x - 5" fill='red'
                       :y="item.y - 15"></use>
                </a-tooltip>
              </template>
              ...
              <template v-for="(item, index) in coincidePoint">
                <a-tooltip :title="item.title" :key="'coincidePoint-'+ index">
                  <circle :cx="item.x" :cy="item.y" :r="item.r" stroke-width="1" :stroke="item.stroke"
                          :fill='item.fill' :title="item.title || 'tw/mb/xl/tt点' "/>
                </a-tooltip>
              </template>
              ...
              <template v-for="(item, index) in intersectPoint">
                <svg :key="'intersectPoint-'+ index">
                  <a-tooltip :title="item.title">
                    <use v-if="item.type === '耳温'" xlink:href="#icon-kongxinsanjiao" width="10"
                         height="10" :x="item.x - 5" fill='blue' :y="item.y - 5"></use>
                    <use v-else xlink:href="#icon-tiwen-03" width="10" height="10" :x="item.x - 5"
                         :fill='item.stroke' :y="item.y - 5"></use>
                  </a-tooltip>
                </svg>
              </template>
              ...
              <template v-for="(item, index) in ttPoint">
                <a-tooltip :title="item.title" :key="'ttPoint-'+ index">
                  <use xlink:href="#icon-shixinsanjiao" width="10" height="10" :x="item.x - 5" fill='red'
                       :y="item.y - 5"></use>
                </a-tooltip>
              </template>
              ...
              <template v-for="(item, index) in fcttPoint">
                <a-tooltip :title="item.title" :key="'fcttPoint-'+ index">
                  <use xlink:href="#icon-kongxinsanjiao" width="10" height="10" :x="item.x - 5" fill='red'
                       :y="item.y - 5"></use>
                </a-tooltip>
              </template>
              ...
              <template v-for="(item, index) in arrowhead">
                <svg :key="'arrowhead' + index">
                  <line :x1="item.x1" :y1="item.y1" :x2="item.x2" :y2="item.y2" :stroke="item.color"
                        :stroke-width="1.3" marker-end='url(#markerArrow)'/>
                  <defs>
                    <marker id='markerArrow' markerWidth='8' markerHeight='8' refX='4' refY='4.5'
                            orient='auto'>
                      <path d='M1.62,1.62 L1.62,7.92 L7.2,4.32 L1.62,1.62' :fill="item.color"
                            :title="item.title || ''"/>
                    </marker>
                  </defs>
                </svg>
              ...
              <template v-for="(item, index) in eventText">
                <svg v-if="item instanceof Array" :key="'eventText' + index">
                  <text v-for="(item2, index) in item" :key="'eventTextText' + index" :x="item2.x"
                        :y="item2.y" :fill="item2.fill" style="font-size:11px"
                        :title="item.title || '事件'">{{item2.text}}
                  </text>
                </svg>
                <text v-else :key="'eventTextText' + index" :x="item.x" :y="item.y" :fill="item.fill"
                      style="font-size:11px" :title="item.title || '事件'">{{item.text}}
                </text>
              </template>
...
export default {
  ...
  data () {
    return {
      ...
      signItemGroup: [], // 分组后的体征项
      showSignItemLength: 0, // 体征项个数
      pointLineData: [] // 体温单数据
    }
  },
  watch: {
  ...
  methods: {
    ...
    parseProps () {
      ...
      // 格式化解析体征项，按顶级父类分组
      // 如: 瞳孔:[反应-右，反应-左，大小-右，大小-左]
      let signItemGroup = {}

      for (let i = 0; i < this.signItem.length; i++) {
        let item = this.signItem[i]

        // 不满足    // 始终显示(1) || 有内容显示(2) && 存储方式不能为空 && 存储方式不为空的
        // let f = this.isShow(item)
        // if (!this.isShow(item, data)) continue

        // 创建分组
        if (!signItemGroup[item['signItemFieldName']]) {
          signItemGroup[item['signItemFieldName']] = { name: '', child: [] }
        }
        signItemGroup[item['signItemFieldName']]['name'] = item['signItemShowName']

        if (item['signItemFieldName'] === 'xueya') { // 血压
          let shousuoyaCache = { data: [] } // 收缩压
          let shuzhangyaCache = { data: [] } // 舒张压
          let celiangweizhiCache = { data: [] } // 测量位置
          let xueyaCache = [] // 血压
          ...
          for (let j = 0; j < item.childrenSignItem.length; j++) {
            ...
            // 测量位置
            if (_d['signItemFieldName'] === 'celiangweizhi') {
              _d['data'] = this.parseLineData(_d, _data)
              celiangweizhiCache = _d
            }
          }
          // 合并收缩压|舒张压
          for (let i = 0; i < shousuoyaCache.data.length; i++) {
            ...
            let type = celiangweizhiCache.data[i]
              ? (celiangweizhiCache.data[i].signValue && celiangweizhiCache.data[i].signValue === '下肢' ? '(下)' : '')
              : ''
            xueyaCache.push({
              signValue: ssy + (ssy ? '/' : '') + szy + type,
              itemValueType: shousuoyaCache.data[i].itemValueType || ''
            })
          }

          if (this.isShow(item, xueyaCache)) {
            item['newSignItemShowName'] = item['signItemShowName']
            signItemGroup[item['signItemFieldName']]['child'].push(
          ...
          this.parseItemGroup(item.childrenSignItem, signItemGroup[item['signItemFieldName']]['child'], sheetData)
        } else {
          let _data = sheetData[item['signItemFieldName']] || []
      ...
      // 底部显示体征项的总个数，对比补空行用
      this.showSignItemLength = 0
      for (let p in signItemGroup) {
        this.showSignItemLength += signItemGroup[p].child.length
      }

      // 分组后的体征项
      this.signItemGroup = signItemGroup
        ...
        for (let j = 0; j < evens.length; j++) {
          ...
          if (finalValue === '请假') {
            ...
            if (minute > 0) {
              ...
              finalValue += '||' + this.numToChinese(hourStr, chinese) + '时'
            }
          }
