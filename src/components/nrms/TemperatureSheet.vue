    ...
    <div style="position: relative">
      ...
      <table class="tableSvg">
        ...
        <tr>
          ...
          <td colspan="6" class="every" v-for="i in 7">{{cur_week_date[i - 1] || ""}}</td>
        </tr>

        <tr>
          <td class="every" colspan="2">住院天数</td>
          <td colspan="6" class="every" v-for="i in 7">{{cur_week_in[i - 1] || ""}}</td>
        </tr>

        <tr>
          <td class="every" colspan="2">手术后天数</td>
          <td colspan="6" class="every" v-for="i in 7">{{cur_week_ss[i - 1] || ""}}</td>
        </tr>
        ...
        <tr class="height hour">
          ...
          <template v-for="i in 7">
            <template v-if="hours.length === 0">
              <td v-for="n in 6" :class="{borderR : n === 6 && i !== 7}"></td>
            </template>
            <template v-else>
              <td v-for="(h, index) in hours" :class="{borderR : index === 5 && i !== 7}" :style="{color: h.temperatureItemColor}" :key="i +'' +h.temperatureConfigId">{{h.takeTemperatureTime}}</td>
            </template>
          </template>
        </tr>
        ...
        <!-- 补空行 -->
        <tr v-for="n in (10 - show_sign_item.length < 0 ? 0 : 10 - show_sign_item.length)">
    ...
    parseRePoint (pointData, beginDate) {
      ...
      for (let i = 0; i < coor.length; i++) {
        ...
        for (let j in json) {
          ...
          // 复测体温
          if (ismbfcwd && mbY === fcwdY && fcgw) { // 脉搏+复测肛温[红圆+蓝圈]
