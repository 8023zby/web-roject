<template>
  <div class="passport-imgUpload">
    <el-upload class="avatar-uploader" action="false" :on-change="handleChange" :multiple="false" :before-upload="beforeAvatarUpload" accept=".png,.jpg">
...
/*
* action="false"
* 实际是文件没有进行上传，只是得到file文件，将file内容，放入formData里面，传给后台，
* 后台处理文件;
* 图片显示需要创建一个url
*/
...
export default {
  ...
  methods: {
    ...
    async handleChange (file, fileList) { // 上传文件让第二次覆盖第一的文件
      console.log(2)

      let _f = []
      // 因为 on-change 添加文件、上传成功和上传失败时都会被调用
      if (fileList.length > 0) {
        // 创建url,用于图片的返显
        let url = URL.createObjectURL(file.raw)

        this.imageObj.imageUrl = url
        _f = [fileList[fileList.length - 1]]

        this.$emit('changeFile', _f)
      }
    }
  }
}
