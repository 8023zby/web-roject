<template>
  <div style="position: relative; height: 100%; background-color: #fff">
    ...
    <div class="nrms_patient_list">
      ...
      <div class="list">
        <el-tabs v-model="activeName" style="height: 100%" v-uni="'feiyongguanli_hzyl_changType'"  @tab-click="handleClick">
          <el-tab-pane label="在院" name="in">
            <el-table
                    ...
                    v-uni="'feiyongguanli_hzyl_zaiyaun_getDetail'"
            >
              <el-table-column
          ...
          <el-tab-pane label="转科" name="trans">
            <el-table
                    ...
                    v-uni="'feiyongguanli_hzyl_zhuanke_getDetail'"
            >
              <el-table-column
                      prop="bedName"
                      label="床号"
          ...
          <el-tab-pane label="出院" name="out">
            <el-table
                    ...
                    v-uni="'feiyongguanli_hzyl_chuyuan_getDetail'"
            >
              <el-table-column
                      prop="bedName"
                      label="床号"
              ...
              >
                <template slot-scope="scope">
                  <span v-show="scope.row.outHospitalFlag === 1" class="isRecall"></span>
...
<script>
import { ApiPatient } from '../../api/bnms'
...
export default {
  ...
  methods: {
    ...
    getInPatientInfo (row) {
      this.$emit('initPatient', row ? row.patientInViewDTO || { patientId: '' } : { patientId: '' }, 0)
    },
    getTransPatientInfo (row) {
      this.$emit('initPatient', row || { patientId: '' }, 1)
    },
    getOutPatientInfo (row) {
      this.$emit('initPatient', row || { patientId: '' }, 2)
    }
  },
  components: {}
}
