...
<template>
  <div class="container yule-main">
    ...
    <div class="main-panel">
      <search-bar :editType="editType" v-on:musicSearch = "musicSearch"></search-bar>
      <edit-bar :editType="editType" v-on:addMusic = "addMusic" v-on:sortMusicType = 'sortEbook' :typeId="typeId"></edit-bar>
      <div class="div-table">
      <el-table
              ref="singleTable"
              :data="tableData"
...
import { editMusic, delMusic, delMusicType, sortMusicTypes, sortMusic } from '@/api/msms/music/index.js'
...
export default {
  ...
  methods: {
    ...
    delEbook () {
      delMusic(this.rowEditId).then(res => {
        if (res.status === 200) {
          this.$message({
            message: '删除成功',
            type: 'success'
    ...
    onSortSubmit () {
      ...
      sortMusicTypes(indexArr).then(res => {
        if (res.status === 200) {
          // 新增成功
