  ...
  <layout_aside v-loading="loading">
    <div slot="main">
      <layout_list>
        <div slot="query" class="query-items">
          <el-form v-uni="'jianyanxiangmu_form'" class="demo-form-inline" ref="search_form" :inline="true" :model="search_form"
                   label-suffix=":" label-width="120px">
            <el-form-item label="检验项目代码" prop="crisisCode">
              <el-input
                      v-uni="'jianyanxiangmu_daima'"
                      placeholder="请输入"
            ...
            <el-form-item label="检验项目名称" prop="crisisName">
              <el-input
                      v-uni="'jianyanxiangmu_mingcheng'"
                      placeholder="请输入"
                v-model="search_form.crisisName"
                clearable></el-input>
            </el-form-item>
            <el-form-item>
              <el-button v-uni="'jianyanxiangmu_search'" type="primary" class="el-button-radius btn-position" icon="el-icon-search"
                         @click="getData()"
                         size="small">搜索
              </el-button>

              <el-button v-uni="'jianyanxiangmu_reset'" type="text" @click="reset" style="margin-left: 0">重置</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div slot="left_btn" class="tool-bar">
          <el-button type="warning" icon="el-icon-plus" @click="itemAdd(null)" v-checkAuth="'addExamineDictionary'"
          >添加
          </el-button>
        </div>
        <div slot="right_btn" class="tool-bar">
          <el-button v-uni="'jianyanxiangmu_import'"  type="primary" @click="openImportDialog" v-checkAuth="'importExamineDictionary'">导入</el-button>
          <el-button v-uni="'jianyanxiangmu_export'" type="primary" @click="exportExcel" v-checkAuth="'exportExamineDictionary'">导出
          </el-button>
        </div>
        <div slot="main" class="main-list">
          <el-table ref="table" highlight-current-row :data="tableData" height="100%"
            ...
            <el-table-column align="center" prop="crisisCode" label="检验项目代码" min-width="200px">
            </el-table-column>
            <el-table-column align="center" prop="crisisName" label="检验项目名称" min-width="200px">
            </el-table-column>
            <el-table-column align="center" prop="crisisShowName" label="显示名称" min-width="200px">
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip min-width="400px">
            ...
            <el-table-column fixed="right" label="操作" width="200" align="center" v-if="checkAuth('editDangerDict,deleteDangerDict')">
              <template slot-scope="scope">
                <el-button
                        v-uni:[scope.row.crisisDictId]="'jianyanxiangmu_edit'"
                        type="text"
                  @click="itemAdd(scope.row)"
                  v-checkAuth="'editExamineDictionary'"
                >编辑
                </el-button>
                <el-button
                        v-uni:[scope.row.crisisDictId]="'jianyanxiangmu_delete'"
                  type="textred"
                  @click="itemDelete(scope.row)"
                  v-checkAuth="'deleteExamineDictionary'"
                >删除
                </el-button>
              </template>
            </el-table-column>
      ...
      <el-dialog
        ...
        <div class="flexContent">
          <el-upload
            ...
            :limit="1"
            :auto-upload="false"
            :data="{importType:'EXAMINE_IMPORT'}"
          >
            <div slot="trigger" class="file">
              <div style="width: 50px;">文件:</div>
              <el-input
                      v-uni="'jianyanxiangmu_importFile'"
                placeholder=""
                autocomplete="off"
                readonly
                v-model="fileName"
              ></el-input>
              <el-button v-uni="'jianyanxiangmu_selectFile'" size="small" style="margin-left: 12px;" type="primary"
              >选取文件
              </el-button>
            </div>
            <div class="process" v-show="progressShow">
              <el-progress v-uni="'jianyanxiangmu_progress'" :percentage="percentage"
                           :show-text="false"></el-progress>
            </div>
            <div slot="tip" class="limitText">支持文件格式EXCEL,文件不能超过5MB。<a
              href="/resource/zhhl_crisis/检验项目导入模板.xlsx">下载导入模板</a></div>
          </el-upload>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button v-uni="'jianyanxiangmu_cancel'" @click="close">取消</el-button>
          <el-button v-uni="'jianyanxiangmu_import'" type="primary" @click="importFiles">导入</el-button>
      </span>
      </el-dialog>
    </template>
    <!--导入报错弹窗-->
    <template>
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <el-button v-uni="'jianyanxiangmu_close'" type="primary" @click="closeErrorDialog">关闭</el-button>
      </span>
      </el-dialog>
    </template>
  </layout_aside>
  ...
  methods: {
    ...
    itemDelete (row) {
      ...
      this.$confirm('确认要进行删除吗？', '删除', {
        ...
        ApiDangerDictionary.delete({ crisisDictId: _row.crisisDictId, crisisType: 2 })
          .then(() => {
    ...
    exportExcel () {
      window.open('/nrms/web-nr/crisis-dict/examine-export?crisisCode=' + this.search_form.crisisCode + '&crisisName=' + this.search_form.crisisName + '&crisisType=2')
    },
    // 导入成功回调
