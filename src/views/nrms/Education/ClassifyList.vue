<template>
  <layout>
    <div slot="left_btn" class="tool-bar">
      <el-button type="warning" size="small" @click="add()" icon="el-icon-plus" v-if="addFormData.parentId!==''">添加</el-button>

    </div>
    <div slot="right_btn" class="tool-bar">
      <el-button type="primary" size="small" @click="addParent">所属分类名称编辑</el-button>
      ...
      >
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="downDo(scope.row, scope.$index)">下移</el-button>
            <el-button type="text" @click="btmDo(scope.row, scope.$index)">置尾</el-button>
    ...
    <template slot>
      <!--添加-->
      <el-dialog title="添加分类" :visible.sync="setFormVisible" width="500px" :close-on-click-modal="false">
        <el-form :rules="rules" :model="addFormData" ref="addForm" label-width="100px" class="typt-add-dialog">
          <el-form-item label="分类名称：" prop="typeName">
            <el-input v-model="addFormData.typeName" autocomplete="off"></el-input>
          </el-form-item>
        </el-form>
      ...
      <!--end-->
      <!--添加-->
      <el-dialog title="所属分类" :visible.sync="parentFormVisible" width="500px" :close-on-click-modal="false">
        <el-form :rules="rules" :model="saveNewData" ref="addParentForm" label-width="130px" class="typt-add-dialog">
          <el-form-item label="所属分类名称：" prop="typeName">
            <el-input v-model="saveNewData.typeName" autocomplete="off"></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="parentFormVisible = false">取消</el-button>
          <el-button type="primary" @click="saveParentData">确认</el-button>
        </div>
      </el-dialog>
      <!--end-->
    </template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        url: '/nrms/web-nr/education-type',
        modelName: '分类',
        rules: {
          typeName: [
    ...
    methods: {
      // 请求父类
      getPidData() {
        AxiosApi('/nrms/web-nr/education-type/query-parent', {}, 'GET', { deptId: this.addFormData.deptId }).then(res => {
