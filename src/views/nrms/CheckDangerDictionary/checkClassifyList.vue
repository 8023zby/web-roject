<template>
  <div class="page-aside-main">
    <div class="left-aside">
      <div class="l-title">检查类型</div>
      <div class="l-action">
        <el-button v-uni="'jianchaxiangmu_tianjiafenlei'" type="primary" @click="addShow(null)" v-checkAuth="'addDangerDictClass'">添加分类</el-button>
        <el-button v-uni="'jianchaxiangmu_sort'" type="primary" @click="toSort()" v-checkAuth="'sortDangerDictClass'" style="float: right;">排序</el-button>
      </div>
      <div class="l-content">
              ...
              <el-col :span="7">
                <div :style="rowHover === index?'display:inline-block':'display:none'"
                  ...
                  <i
                    ...
                    v-checkAuth="'editDangerDictClass'"
                                    ></i>
                  ...
                  <i
                    ...
                    v-checkAuth="'deleteDangerDictClass'"
                  ></i>
                </div>
              </el-col>
            </el-row>
    ...
    <el-dialog
      ...
      custom-class="classifyDialogStyle">
              ...
              <el-form
                      v-uni="'jianchaxiangmu_tianjiafenleiform'"
                      :model="classify_form"
                      :rules="rules"
                      ref="classify_form"
                      label-width="100px"
              ...
              >
                  <el-form-item label="分类名称" prop="classValue">
                      <el-input
                              v-uni="'jianchaxiangmu_fenleimingcheng'"
        ...
        <el-button v-uni="'jianchaxiangmu_fenleicancel'" @click="close">取消</el-button>
        <el-button v-uni="'jianchaxiangmu_fenleiconfirm'" type="primary" @click="addClassify">确认</el-button>
    </span>
          </el-dialog>
    </template>
    <!--排序弹窗-->
    ...
    >
        ...
        <el-table
          ref="dragTable"
          :data="sort_data"
          style="width: 100%;padding: 0 50px 35px;"
          row-key="id"
          fit
          stripe
        ...
        >
          ...
          <el-table-column label="标题" prop="classValue" />
          <el-table-column label="操作" width="420">
            <template slot-scope="scope">
              <el-button v-uni:[scope.row.crisisClassId]="'jianchaxiangmu_head'" type="text" @click="itemSort(scope.$index, scope.row, 'top')">置顶</el-button>
              <el-button v-uni:[scope.row.crisisClassId]="'jianchaxiangmu_up'" type="text" @click="itemSort(scope.$index, scope.row, 'up')">上移</el-button>
              <el-button v-uni:[scope.row.crisisClassId]="'jianchaxiangmu_down'" type="text" @click="itemSort(scope.$index, scope.row, 'down')">下移</el-button>
              <el-button v-uni:[scope.row.crisisClassId]="'jianchaxiangmu_bottom'" type="text" @click="itemSort(scope.$index, scope.row, 'bottom')">置底</el-button>
              <el-button v-uni:[scope.row.crisisClassId]="'jianchaxiangmu_setposition'" type="text" @click="itemSort(scope.$index, scope.row, 'assign')">指定位置</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div slot="footer" class="dialog-footer">
          <el-button v-uni="'jianchaxiangmu_sortcancel'" @click="dialogSortVisible = false">取消</el-button>
          <el-button v-uni="'jianchaxiangmu_sortconfirm'" type="primary" @click="onSortSubmit()">确认</el-button>
        </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    itemDelete (row) {
      ...
      ApiDangerClassify.deleteSelect({ crisisClassId: _row.crisisClassId, classType: '1' })
        .then((res) => {
          let result = res.data
          if (result) {
            // 为true时可以删除
            this.$confirm('确认要进行删除吗？', '删除', {
              ...
              ApiDangerClassify.delete({ crisisClassId: _row.crisisClassId, classType: '1' })
                .then(() => {
