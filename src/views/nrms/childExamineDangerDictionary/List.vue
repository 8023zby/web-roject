  ...
  <layout_aside v-loading="loading">
    ...
    <div slot="main">
      <layout_list>
        <div slot="query" class="query-items">
          <el-form v-uni="'jianyanzixiangmu_form'" class="demo-form-inline" ref="search_form" :inline="true" :model="search_form"
                   label-suffix=":" label-width="120px">
            <el-form-item label="检验子项目代码" prop="crisisChildCode">
              <el-input
                      v-uni="'jianyanzixiangmu_daima'"
                placeholder="请输入"
                v-model="search_form.crisisChildCode"
                clearable></el-input>
            </el-form-item>
            <el-form-item label="检验子项目名称" prop="crisisChildName">
              <el-input
                      v-uni="'jianyanzixiangmu_mingcheng'"
                placeholder="请输入"
                v-model="search_form.crisisChildName"
                clearable></el-input>
            </el-form-item>
            <el-form-item>
              <el-button  v-uni="'jianyanzixiangmu_search'" type="primary" class="el-button-radius btn-position" icon="el-icon-search"
                         @click="getData()"
                         size="small">搜索
              </el-button>

              <el-button  v-uni="'jianyanzixiangmu_reset'" type="text" @click="reset" style="margin-left: 0">重置</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div slot="left_btn" class="tool-bar">
          <el-button v-uni="'jianyanzixiangmu_add'" type="warning" icon="el-icon-plus" @click="itemAdd(null)" v-checkAuth="'addchildExamineDictionary'"
          >添加
          </el-button>
        </div>
        <div slot="right_btn" class="tool-bar">
          <el-button v-uni="'jianyanzixiangmu_import'" type="primary" @click="openImportDialog" v-checkAuth="'importChildExamineDictionary'">导入</el-button>
          <el-button v-uni="'jianyanzixiangmu_export'" type="primary" @click="exportExcel" v-checkAuth="'exportChildExamineDictionary'">导出
          </el-button>
        </div>
        <div slot="main" class="main-list">
          <el-table ref="table" highlight-current-row :data="tableData" height="100%"
            ...
            <el-table-column align="center" prop="crisisChildCode" label="检验子项目代码" min-width="150px">
            </el-table-column>
            <el-table-column align="center" prop="crisisChildName" label="检验子项目名称" min-width="150px">
            </el-table-column>
            <el-table-column align="center" prop="crisisChildShowName" label="显示名称" min-width="150px">
            </el-table-column>
            <el-table-column align="center" prop="crisisChildLoinc" label="LONIC代码" min-width="150px">
            </el-table-column>
            <el-table-column align="center" prop="crisisChildEnName" label="英文缩写" min-width="150px">
            </el-table-column>
            <el-table-column align="center" prop="crisisChildUnit" label="单位" min-width="150px">
            </el-table-column>
            <el-table-column align="center" show-overflow-tooltip>
              <template slot="header" slot-scope="scope">项目备注</template>
              <template slot-scope="scope">
                {{scope.row.crisisChildNote}}
              </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="200" align="center" v-if="checkAuth('editDangerDict,deleteDangerDict')">
              <template slot-scope="scope">
                <el-button
                        v-uni:[scope.row.crisisChildDictId]="'jianyanzixiangmu_edit'"
                  type="text"
                  @click="itemAdd(scope.row)"
                  v-checkAuth="'editChildExamineDictionary'"
                >编辑
                </el-button>
                <el-button
                        v-uni:[scope.row.crisisChildDictId]="'jianyanzixiangmu_delete'"
                  type="textred"
                  @click="itemDelete(scope.row)"
                  v-checkAuth="'deleteChildExamineDictionary'"
                >删除
                </el-button>
              </template>
            </el-table-column>
      ...
      <el-dialog
        ...
        <div class="flexContent">
          <el-upload
            ...
            :limit="1"
            :auto-upload="false"
            :data="{importType:'EXAMINE_CHILD_IMPORT'}"
          >
            <div slot="trigger" class="file">
              <div style="width: 50px;">文件:</div>
              <el-input
                      v-uni="'jianyanzixiangmu_selectFileName'"
                placeholder=""
                autocomplete="off"
                readonly
                v-model="fileName"
              ></el-input>
              <el-button v-uni="'jianyanzixiangmu_selectFile'" size="small" style="margin-left: 12px;" type="primary"
              >选取文件
              </el-button>
            </div>
            <div class="process" v-show="progressShow">
              <el-progress v-uni="'jianyanzixiangmu_progress'"  :percentage="percentage"
                           :show-text="false"></el-progress>
            </div>
            <div slot="tip" class="limitText">支持文件格式EXCEL,文件不能超过5MB。<a
              href="/resource/zhhl_crisis/检验子项目导入模板.xlsx">下载导入模板</a></div>
          </el-upload>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button v-uni="'jianyanzixiangmu_cancel'" @click="close">取消</el-button>
          <el-button v-uni="'jianyanzixiangmu_importFile'"  type="primary" @click="importFiles">导入</el-button>
      </span>
      </el-dialog>
    </template>
    <!--导入报错弹窗-->
    <template>
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <el-button v-uni="'jianyanzixiangmu_close'" type="primary" @click="closeErrorDialog">关闭</el-button>
      </span>
      </el-dialog>
    </template>
  </layout_aside>
...
import { ApiDangerDictionary, ApiChildDangerDictionary } from '../../../api/nrms'
...
export default {
  ...
  methods: {
    ...
    itemDelete (row) {
      ...
      ApiChildDangerDictionary.deleteSelect({ crisisChildDictId: _row.crisisChildDictId })
        .then((res) => {
          let result = res.data
          if (result) {
            // 为true时可以删除
            this.$confirm('确认要进行删除吗？', '删除', {
              ...
              ApiChildDangerDictionary.delete({ crisisChildDictId: _row.crisisChildDictId })
                .then(() => {
            ...
            this.$confirm('该检验子项目与危急值设置关联，无法删除！', '删除', {
    ...
    exportExcel () {
      window.open('/nrms/web-nr/crisis-child-dict/export?crisisClassId=' + this.search_form.crisisClassId + '&crisisCode=' + this.search_form.crisisChildCode + '&crisisName=' + this.search_form.crisisChildName)
    },
    // 导入成功回调
