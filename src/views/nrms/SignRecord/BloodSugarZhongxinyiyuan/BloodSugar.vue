  ...
  <div class="nrms-main-div">
    <layout class="nrms-blood-list">
      <div slot="right_btn" class="tool-bar" v-if="!isPreview">
        <el-button v-uni="'xuetangdan_qushitu'" type="primary" size="small" @click="$emit('option-changed', 'blood_sugar_chart')">血糖单趋势图</el-button>
        <el-button v-uni="'xuetangdan_print'" type="success" size="small" @click="printDo">打印</el-button>
      </div>
      <div slot="main" class="main-list">
        ...
        >
          ...
          <template v-if="!btn_disable">
            ...
            >
              <template slot-scope="scope">
                <el-button type="text" v-uni:[scope.$index]="'xuetangdan_edit'" @click="modify(scope.row)">编辑</el-button>
                <el-button type="textred" v-uni:[scope.$index]="'xuetangdan_delete'" @click="del(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </template>
        </el-table>
      ...
      <div style="display: none">
        <print ref="print" :isGlucoseMeter="isGlucoseMeter" :titData="titData" :formId="formObj.formId"
               :patientId="formSearchDataStatic.patientId" :printSet="printSet" :autoHead="autoHead"></print>
      </div>
    </layout>
    <!-- 出院&&非出院召回 || 转科的患者不显示-->
    <template v-if="!btn_disable && !isPreview">
      <div class="el-col-add absolute"
           :style="{right:form_show_hide ? '-437px': 0}" id="add_form">
        <el-button size="small" class="form_show_btn" v-uni="'xuetangdan_expend'" @click="formShowHide">
          <i :class="form_show_hide ? 'el-icon-arrow-left' : 'el-icon-arrow-right'"></i>
        </el-button>
        <el-form class="nrms-blood-add" :model="addFormData" :rules="rules" ref="addForm" label-width="98px">
          <el-form-item label="时间:" prop="recordTime">
            <el-date-picker
              v-uni="'xuetangdan_time'"
          ...
          <el-form-item label="测量时刻:" prop="measurePeriod">
            <el-select v-uni="'xuetangdan_celiangshike'" v-model="addFormData.measurePeriod" placeholder="请选择" size="small" clearable>
          ...
          <el-form-item label="测量结果:" prop="measureResult">
            <el-input v-uni="'xuetangdan_celiangjieguo'" v-model="addFormData.measureResult" placeholder="测量结果" style="margin-right: 7px;"
                      clearable></el-input>
            mmol/L
          </el-form-item>
          <el-form-item label="血糖仪编号:" prop="glucometerNumber" v-if="isGlucoseMeter">
            <el-input v-uni="'xuetangdan_xuetangyibianhao'" v-model="addFormData.glucometerNumber" placeholder="请输入" clearable></el-input>
          </el-form-item>
          <!-- 2020-12-25 世博血糖单增加备注字段 -->
          <template v-if="autoHead.tplId === 'H-000010-WXT-0001'">
            <el-form-item label="备注:" prop="remark">
              <el-input type="textarea"
                        v-uni="'xuetangdan_beizhu'"
                        maxlength="50"
                        show-word-limit
        ...
        <div class="typt-add-footer btn_footer" v-if="!isPreview">
          <el-button @click="reloadUrl" v-uni="'xuetangdan_cancel'">取消</el-button>
          <el-button type="primary" @click="saveData" :loading="addVis" v-uni="'xuetangdan_save'">保存</el-button>
        </div>
      </div>
    </template>
  </div>
...
import uniId from './uni-id.js'
import Vue from 'vue'
Vue.directive('uni-id', uniId)
