...
import { ApiBloodChartData } from '../../../../api/nrms/index'
export default {
  name: 'bloodSugarChartShibo',
  ...
  created () {
    ...
    this.search_form.formCode = this.params.formCode
    this.getData()
  },
  methods: {
    getData () {
      ...
      this.valueData = []
      this.loading = true
      ApiBloodChartData.selectNew(this.search_form).then(res => {
        let list = res.data || []
    ...
    dateChange () {
      if (moment(this.search_form.begin_date).isAfter(this.search_form.end_date)) {
