<template>
  <el-container class="passport-login-div-new">
    ...
    <el-main class="passport-login-main-div-new">
      <img src="/static/passport/img/bgNew.png" class="passport-login-bg"/>
      <div class="passport-logo-container">
        <img :src="systemLogo"/>
      </div>
      <el-container class="passport-login-form-div">
        <el-aside class="passport-login-form-img">
          <img :src="loginBackground"/>
          <div class="passport-left-tit">
            <div class="passport-left-tit-item">{{systemNameNew}}</div>
          </div>
        </el-aside>
        <el-main class="passport-login-form-main">
          <div v-if="isShowRegister">
            ...
            <el-form :model="formRegister" :rules="ruleRegister" ref="formRegister" class="regStyle regStyleNew">
          ...
          <el-form v-else ref="loginForm" :rules="rules" :model="loginFormFields" :class="{'passport-login-pl':true,'passport-no-code':!isCheckCode}">
            ...
            <el-row>
              <el-col :span="24">
                <el-button @click="submitForm()" class="passport-submit" type="primary" :loading="logining">立即登录</el-button>
              </el-col>
            </el-row>
    ...
    <el-dialog title="选择科室" :visible.sync="showUnitVis" :show-close="false" width="490px" :close-on-click-modal="false">
      <el-form :rules="rules_unit" :model="unitData" label-width="90px" ref="unitSelect" class="typt-add-dialog">
        <el-form-item v-if="landingEndList.length > 1" label=" 登录应用：" prop="landingEndId">
          <el-select v-model="unitData.landingEndId"  placeholder="请选择" clearable>
            <el-option v-for="(item,index) in landingEndList" :key="index" :label="item.landingEndName" :value="item.landingEndId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item v-if="dataDeptInfoList.length > 1 && globalConfig.orgType === 2" label="科室：" prop="deptId">
          <el-select v-model="unitData.deptId"  placeholder="请选择" clearable>
...
export default {
  ...
  data () {
    return {
      systemLogo: '/static/passport/img/tit_yarward.png',
      systemNameNew: '',
  ...
  methods: {
    // 获取背景图
    getSystemLogin () {
      AxiosApi('/passport/web-rbac/systemConfigs/loginConfig', {}, 'GET').then(res => {
        if (res.status === 200) {
          let data = res.data
          if (data !== null) {
            this.systemLogo = (data.loginIcon === null || data.loginIcon === '') ? '/static/passport/img/tit_yarward.png' : data.loginIcon
            this.systemNameNew = (data.loginIconName === null || data.loginIconName === '') ? this.globalConfig.systemName : data.loginIconName
            this.loginBackground = (data.loginBackground === null || data.loginBackground === '') ? '/static/passport/img/leftImgNew.jpg' : data.loginBackground
          }
        }
      })
    }
...
<style lang="scss">
  ...
  .passport-left-tit-item{
    position: relative;
  }
  .passport-left-tit-item::after{
    content: "";
    position: absolute;
    right: 0;
    bottom: -40px;
