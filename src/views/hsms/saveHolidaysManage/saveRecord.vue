<template>
  <div style="position: relative; height: 100%; background-color: #fff">
    ...
    <div class="nrms_nurse_list">
      ...
      <div class="list">
        <div class="right-btn">
          <el-button type="primary" v-uni="'cunjiaguanli_export'" @click="exportExcel">导出</el-button>
        </div>
        <el-table ref="table" highlight-current-row :data="tableData" height="100%" :row-style="{height:'36px'}" stripe :row-class-name="tableRowClassName" v-uni="'cunjiaguanli_nurse_click'" @row-click="getAccountInfo">
          ...
          <el-table-column align="center" prop="workOvertime" label="加班累积" width="90px" show-overflow-tooltip>

          </el-table-column>
          <el-table-column align="center" prop="workDeduction" label="扣班累积" width="90px" show-overflow-tooltip>
          </el-table-column>
          <el-table-column align="center" prop="holidayBalance" label="存假余额" width="90px" show-overflow-tooltip>
          </el-table-column>
        </el-table>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getData() {
      ApiStorgeHolidays.selectStorageSum({ accountType: 2 })
        .then((res) => {
          this.$nextTick(() => {
    ...
    exportExcel() {
      window.open(
        "/hsms/web-hs/storage-holiday/export?accountType=2&deptName=" +
          this.deptName
      );
    },
