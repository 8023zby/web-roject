  ...
  <layout_aside v-loading="loading">
        ...
        <div slot="query" class="query-items">
          <el-form ref="search_form" :inline="true" :model="search_form"
            ...
            <el-form-item label="生效日期" prop="dateType">
              <el-radio-group v-model="search_form.dateType" v-uni="'nianjiaguanli_changeDate'" @change="dateTypeChange">
            ...
            <el-form-item prop="startDate">
              <el-col :span="11">
                <el-date-picker
                  ...
                  :disabled="(search_form.dateType===1)||(search_form.dateType===2)"
                >
                </el-date-picker>
              </el-col>
            </el-form-item>
            ...
            <el-form-item>
              <el-button type="primary" class="el-button-radius btn-position" icon="el-icon-search"
                         @click="getData()"
                         size="small" v-uni="'nianjiaguanli_search'">搜索
              </el-button>

              <el-button type="text" @click="reset" v-uni="'nianjiaguanli_reset'">重置</el-button>
            </el-form-item>
          </el-form>
        </div>
        <div slot="left_btn" class="tool-bar">
          <el-button type="warning" icon="el-icon-plus" @click="itemAdd(null)"
           v-uni="'nianjiaguanli_add'">添加
          </el-button>
        </div>
        <div slot="main" class="main-list">
          <el-table ref="table" highlight-current-row :data="tableData" height="100%"
                    :row-style="{height:'48px'}" stripe>
            ...
            <el-table-column align="center" prop="holidayBalanceValueStr" label="未休天数" min-width="150px">
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="150" align="center">
              <template slot-scope="scope">
                ...
                <template v-else>
                  <el-button v-uni[scope.row.id]="'nianjiaguanli_edit'" :key="'nianjiaguanli_edit' + scope.row.id" @click="itemAdd(scope.row)" type="text"
                  >编辑
                  </el-button>
                  <el-button v-uni[scope.row.id]="'nianjiaguanli_delete'" :key="'nianjiaguanli_delete' + scope.row.id" type="textred" @click="itemDelete(scope.row)"
                  >删除
                  </el-button>
                </template>
              </template>
    ...
    <template>
      <el-dialog
        ...
        :visible.sync="addHolidaysDialogVisible"
        width="490px"
        :show-close="false"
        :close-on-click-modal="false"
        custom-class="holidaysDialogStyle">

        <el-form
          :model="save_holidays_form"
          :rules="rules"
          ref="save_holidays_form"
          label-width="100px"
          label-suffix=":"
        >
          <el-form-item label="类型" prop="holidayType" class="firstFormItem">
            <el-radio-group v-model="save_holidays_form.holidayType">
              <el-radio :label="1">增加年假</el-radio>
              <el-radio :label="2">扣除年假</el-radio>
            </el-radio-group>
          </el-form-item>
          <div class="addOrReduceTime">
            <el-form-item prop="holidayDay">
              ...
              <el-input
                v-model="save_holidays_form.holidayDay"
                clearable
                placeholder="请输入"
                autocomplete="off"
                maxlength="3"
              ></el-input><div style="line-height: 46px;">天</div>
            </el-form-item>
            <el-form-item prop="holidayHour">
              <el-input
                v-model="save_holidays_form.holidayHour"
                clearable
                placeholder="请输入"
                autocomplete="off"
                maxlength="3"
              ></el-input><div style="line-height: 46px;">小时</div>
            </el-form-item>
          </div>
          <el-form-item label="生效日期" prop="effectiveDate" class="inputW">
            <el-date-picker
              v-model="save_holidays_form.effectiveDate"
              type="date"
              value-format="yyyy-MM-dd"
              placeholder="请选择"
              :clearable="false"
              :picker-options="pickerBeginValueOptions"
              >
            </el-date-picker>
          </el-form-item>
          <el-form-item label="备注" prop="remark" class="inputW">
            <el-input
              type="textarea"
              :rows="5"
              v-model="save_holidays_form.remark"
              maxlength="100"
              show-word-limit
              placeholder="请输入"
            ></el-input>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button v-uni="'nianjiaguanli_cancel'" @click="close">取消</el-button>
          <el-button v-uni="'nianjiaguanli_save'" type="primary" @click="addHoliday">确认</el-button>
      </span>
      </el-dialog>
    </template>
  </layout_aside>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        holidayDay: [
          { validator: validateHolidayDay, trigger: 'blur' }
        ],
        holidayHour: [
          { validator: validateHolidayHour, trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    dateTypeChange (val) {
      ...
      switch (val) {
        case 1:
          ...
          this.search_form.startDate = parseInt(paramYear - curYear) < 0 ? moment().format('YYYY') + '-01-01' : moment().subtract(1, 'quarters').format('YYYY-MM-DD')
          this.search_form.endDate = moment().format('YYYY-MM-DD') // 结束时间（当前系统的时间）
          break
        case 2:
          paramYear = moment().subtract(6, 'months').format('YYYY')
          this.search_form.startDate = parseInt(paramYear - curYear) < 0 ? moment().format('YYYY') + '-01-01' : moment().subtract(6, 'months').format('YYYY-MM-DD') // 当前时间往前推六个月的时间
          this.search_form.endDate = moment().format('YYYY-MM-DD') // 结束时间（当前系统的时间）
          break
        case 3:
          paramYear = moment().subtract(1, 'months').format('YYYY')
          this.search_form.startDate = parseInt(paramYear - curYear) < 0 ? moment().format('YYYY') + '-01-01' : moment().subtract(1, 'months').format('YYYY-MM-DD') // 当前时间往前推一个月的时间
          this.search_form.endDate = moment().format('YYYY-MM-DD') // 结束时间（当前系统的时间）
          break
        default:
          paramYear = moment().subtract(1, 'quarters').format('YYYY')
    ...
    holidaysTypeChange (val) {
      switch (val) {
        case 1:
          this.addOrReduceHolidays = '增加年假'
          break
        case 2:
          this.addOrReduceHolidays = '扣除年假'
          break
        default:
          this.addOrReduceHolidays = '增加年假'
          break
    ...
    itemAdd (row = null) {
      // 清空
      this.save_holidays_form = JSON.parse(JSON.stringify(this.save_holidays_form_catch))
      ...
      if (row && row['id']) {
        ...
        this.save_holidays_form.effectiveDate = moment(row.effectiveDate).format('YYYY-MM-DD')
        // 将天和小时数值拆分出来
        this.save_holidays_form.holidayDay = row.holidayValueStr ? row.holidayValueStr.split('天')[0] : ''
        this.save_holidays_form.holidayHour = row.holidayValueStr ? row.holidayValueStr.split('天')[1].split('小时')[0] : ''
      }
    ...
    addHoliday () {
      this.$refs.save_holidays_form.validate((valid) => {
        if (valid) {
          this.save_holidays_form.effectiveDate = this.save_holidays_form.effectiveDate + ' ' + moment().format('HH:mm:ss') // 将时间带上当前时分秒传递给后台
          this.save_holidays_form.holidayValueStr = (this.save_holidays_form.holidayDay ? this.save_holidays_form.holidayDay : 0) + ',' + (this.save_holidays_form.holidayHour ? this.save_holidays_form.holidayHour : 0)
          ApiAnnualHolidays.save(this.save_holidays_form)
            .then(() => {
    ...
    getData () {
      ApiAnnualHolidays.selectQueryPage(this.search_form).then(res => {
    ...
    itemDelete (row) {
      ...
      this.$confirm('确认要进行删除吗？', '删除', {
        ...
        ApiAnnualHolidays.delete({ id: _row.id })
          .then(() => {
  ...
  components: {
    save_record, layout_aside, layout_list
  }
}
</script>
