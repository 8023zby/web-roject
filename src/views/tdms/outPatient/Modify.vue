    ...
    <div class="typt-add-container">
      <div class="typt-add-form tdms-input-item">
        <el-form :model="addForm" :rules="rules" ref="addForm" label-width="130px">
          ...
          <el-form-item label="房间名：" prop="roomId" v-if="roomShow.join(',').indexOf(deviceType)>-1">
          ...
          <el-form-item v-if="areaHidden.join(',').indexOf(deviceType) === -1" label="安装位置：" prop="positionStr" class="deptCls">
            <treeselect style="width: 300px" :disable-branch-nodes="true" :default-expand-level="10" placeholder="请选择" v-model="addForm.positions.positionStr" noResultsText="无匹配数据" noOptionsText="暂无数据" :multiple="false" :options="positionList" noChildrenText="无子节点">
...
export default {
  ...
  watch: {
    deptId (val) {
      ...
      if (this.roomShow.join(',').indexOf(this.deviceType) > -1) {
        this.getRooms()
      }
    }
  },
  ...
  methods: {
    ...
    initModifyData (data) {
      ...
      /* if (this.deviceType === 'orInfoPromptScreen') {
