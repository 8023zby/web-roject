...
<!--      <el-button v-uni="actKey + 'add'" v-if="authorityObj[`add_${formSearchDataStatic.deviceType}`]" class="el-button-tdms-search" type="warning" size="small" @click="$emit('option-changed', 'add',{ deviceType: formSearchDataStatic.deviceType })" icon="el-icon-plus">添加</el-button>-->
    ...
    <el-table
      ...
      <el-table-column prop="macAddress" label="设备绑定用户名" align="center"></el-table-column>
      <el-table-column prop="nfcId" label="NFC" align="center"></el-table-column>
      <el-table-column prop="deviceMac" label="MAC" align="center"></el-table-column>
      <el-table-column prop="icCardNum" label="设备在线状态" align="center"></el-table-column>
...
import ImportExcel from '../../../components/tdms/ImportWatchExcel'
...
export default {
  ...
  data () {
    return {
      ...
      url: '/tdms/web-td/device/watch',
      authorityArr: [],
      firstFlag: true,
      actKey: 'zhihushoubiao_',
      actFormKey: 'zhihushoubiao__'
    }
  },
  activated () {
    this.authorityArr = ['add_' + this.formSearchDataStatic.deviceType, 'mod_' + this.formSearchDataStatic.deviceType,
      'del_' + this.formSearchDataStatic.deviceType, 'import_' + this.formSearchDataStatic.deviceType]
    // 判断登录端
