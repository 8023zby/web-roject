      ...
      <div class="typt-add-form tdms-input-item">
        <el-form :model="addForm" :rules="rules" ref="addForm" label-width="130px">
          <el-form-item label="护理单元：" prop="deptId" class="deptCls">
            <treeselect v-uni="actFormKey + 'hulidianyuan'" style="width: 300px" placeholder="请选择" v-model="deptId" noResultsText="无匹配数据" noOptionsText="暂无数据" :multiple="false" :options="deptList" noChildrenText="无子节点" :disabled="deptIdDisabled.join(',').indexOf(deviceType)>-1"/>
          </el-form-item>
          <el-form-item label="终端id：">
            <el-input v-uni="actFormKey + 'zhongduanid'" v-model="addForm.deviceAppId" disabled></el-input>
          </el-form-item>
          <el-form-item label="终端型号：">
            <el-input v-uni="actFormKey + 'zhongduanxinghao'" v-model="addForm.deviceModel" disabled></el-input>
          </el-form-item>
          <el-form-item label="终端IP：" v-if="ipHidden.join(',').indexOf(deviceType) < 0">
            <el-input v-uni="actFormKey + 'zhongduanip'" v-model="addForm.ip" disabled></el-input>
          </el-form-item>
          <el-form-item label="终端号：" prop="deviceNum">
            <el-input v-uni="actFormKey + 'zhongduanhao'" v-model="addForm.deviceNum" :disabled="deviceNumDisabled.join(',').indexOf(deviceType)>-1"></el-input>
          </el-form-item>
          ...
          <el-form-item label="房间名：" prop="roomId" v-if="deviceType === 'wnDoorWayExtension'">
            <el-select v-uni="actFormKey + 'fangjianming'" style="width: 300px" multiple placeholder="请选择" v-model="addForm.roomId" filterable clearable >
          ...
          <el-form-item label="房间名：" prop="roomId" v-else-if="roomShow.join(',').indexOf(deviceType)>-1">
            <el-select v-uni="actFormKey + 'fangjianming'" style="width: 300px" @change="changeRoom" placeholder="请选择" v-model="addForm.roomId" filterable clearable  :disabled="roomDisabled.join(',').indexOf(deviceType)>-1">
              <el-option v-for="item in roomData" :label="item.roomName" :value="item.roomId"
                         :key="item.roomId">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="安装房间：" prop="InstallationRoomId" v-if="deviceType === 'wnToiletExtension'">
            <el-select v-uni="actFormKey + 'anzhuangfangjianming'" style="width: 300px" placeholder="请选择" v-model="addForm.positions.InstallationRoomId" filterable clearable  >
              <el-option v-for="item in roomData" :label="item.roomName" :value="item.roomId"
                         :key="item.roomId">
              </el-option>
            </el-select>
          ...
          <el-form-item label="床位名：" prop="bedId" v-if="bedShow.join(',').indexOf(deviceType)>-1">
            <el-select v-uni="actFormKey + 'chuangweiming'" style="width: 300px" placeholder="请选择" v-model="addForm.bedId" filterable clearable>
              <el-option v-for="item in bedData" :label="item.bedName" :value="item.bedId"
                         :key="item.bedId">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="areaHidden.join(',').indexOf(deviceType) === -1" label="安装位置：" prop="positionStr" class="deptCls">
            <treeselect v-uni="actFormKey + 'anzhuangweizhi'" style="width: 300px" :disable-branch-nodes="true" :default-expand-level="10" placeholder="请选择" v-model="addForm.positions.positionStr" noResultsText="无匹配数据" noOptionsText="暂无数据" :multiple="false" :options="positionList" noChildrenText="无子节点">
              <div slot="value-label" slot-scope="{ node }">{{ node.raw.allName }}</div>
            </treeselect>
          </el-form-item>
          <el-form-item label="广播功能：" prop="remark" v-if="broadCastShow.join(',').indexOf(deviceType)>-1">
            <el-switch
              v-uni="actFormKey + 'guangbogongneng'"
              v-model="addForm.broadcast"
              active-color="#1D86EF"
          ...
          <el-form-item label="备注信息：" prop="remark">
            <el-input v-uni="actFormKey + 'beizhuxinxi'" style="width: 300px" maxlength="100" show-word-limit placeholder="请输入" type="textarea" :rows="4" v-model="addForm.remark"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  watch: {
    deptId (val) {
      ...
      this.addForm.positions.InstallationRoomId = ''
  ...
  data () {
    return {
      ...
      roomObj: {},
      deptObj: {},
      roomShow: ['wnBedHeadExtension', 'wnBedSideExtension', 'wnDoorWayExtension', 'wnDoorLampExtension', 'wnToiletExtension'],
      bedShow: ['wnBedHeadExtension', 'wnBedSideExtension'],
      broadCastShow: ['wnBedHeadExtension', 'wnBedSideExtension', 'wnDoorWayExtension', /*'wnCorridorScreen', 'wnEntranceGuard'*/, 'rvVisitExtension'/*, 'wnDutyMainframe'*/],
      areaHidden: ['wnMedicalAudioAssistant'],
      ipHidden: ['wnToiletExtension'],
      deptIdDisabled: ['wnToiletExtension'],
      deviceNumDisabled: ['wnToiletExtension'],
      roomDisabled: ['wnToiletExtension'],
      addForm: {
        deviceAppId: '',
        roomId: '',
        bedId: '',
      ...
      rules: {
        ...
        deviceNum: [
          { min: 0, max: 100, message: '终端号长度不超过100位', trigger: 'blur' }
        ]
      },
