        ...
        <div class="middle">
          ...
          <div class="middle_bottom">
            <div class="left">
              ...
              <div>
                <span class="item">核对：{{item.dispenseCheckNurse ? user_data[item.dispenseCheckNurse]: ''}}</span>
...
export default {
  name: 'advice_dispense_batch_print',
  data () {
    return {
      loading: false,
      user_data: [], // 用户信息
  ...
  methods: {
    ...
    getPatientIds () {
      ...
      for (let i = 0; i < this.patient.length; i++) {
        patientIds += this.patient[i]['patientId'] + ','
      }
      this.search_form.patientIds = patientIds.substring(0, patientIds.length - 1)
    },
    getTableData () {
      this.table_data = []
      this.loading = true
    ...
    close () {
      this.$emit('option-changed', 'batchDispense')
    }
  }
}
</script>
