    ...
    <div slot="main" class="main-list">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            {{
            scope.row.medicalAdviceOperationType!==8
            ? user_data[scope.row.executeNurse]
            : (scope.row.skinTestExecuteDtoList && scope.row.skinTestExecuteDtoList[0]
            ? user_data[scope.row.skinTestExecuteDtoList[0].executeNurse]
            : ''
            )
            }}
          </template>
        ...
        >
          <template slot-scope="scope">
            ...
            : (scope.row.skinTestExecuteDtoList && scope.row.skinTestExecuteDtoList[0]
            ? scope.row.skinTestExecuteDtoList[0].executeTime
            : ''
            )
            }}
          </template>
        ...
        <el-table-column label="操作" align="center" fixed="right" width="130">
          <template slot-scope="scope">
            ...
            <!-- 停嘱复核 -->
            <el-button v-uni:[scope.$index]="'yizhuliebiao_quxiaotingzhufuhe'" type="text" @click="adviceStopCheckCancel(scope.row)"
                       v-if="scope.row.checkStopStatus === 1"
            >取消停嘱复核
            </el-button>
            ...
            <!-- 开嘱复核 -->
            <el-button v-uni:[scope.$index]="'yizhuliebiao_quxiaokaizhufuhe'" type="text" @click="adviceStartCheckCancel(scope.row)"
                       :disabled="scope.row.isExecuted"
                       v-if="scope.row.checkStartStatus === 1 && (scope.row.checkStopStatus === 0 || !scope.row.checkStopStatus)"
            >取消开嘱复核
            </el-button>
            <el-button v-uni:[scope.$index]="'yizhuliebiao_kaizhufuhe'" @click="adviceStartCheck(scope.row)" type="text"
                       v-if="scope.row.startAdviceTime &&  (!scope.row.checkStartStatus || scope.row.checkStartStatus === 0 || scope.row.checkStartStatus === 2)"
            >开嘱复核
            </el-button>
            </template>
          </template>
      ...
      ApiDoctorsAdvice.select(this.search_form)
        .then(res => {
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            // 执行护士 时间 (临时医嘱只执行一次，所以只取第一个并放在第一行)
            let executeNurseNameAndTime =
                      list[i].executeNurseNameAndTimeDtoList && list[i].executeNurseNameAndTimeDtoList[0]
                        ? list[i].executeNurseNameAndTimeDtoList[0]
                        : {}
            Object.assign(item, executeNurseNameAndTime, { selfIndex: (this.search_form.page - 1) * this.search_form.size + i + 1 })
