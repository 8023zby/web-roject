  ...
  <div class="print_container" v-loading="loading">
    ...
    <div class="dams_doctors_advice_long_tern_container" id="dams_doctors_advice_long_tern_container">
      <table id="dams_doctors_advice_long_tern_container_table">
        ...
        <tr>
          <td colspan="9" class="title">长期医嘱单</td>
        </tr>
        <tr>
        ...
        <tr v-for="advice in table_data">
          ...
          <td>{{advice.stopAdviceDate}}</td>
          <td>{{advice.stopAdviceTime}}</td>
          <td>{{advice.stopAdviceDoctor || ''}}</td>
          <td>{{user_data[advice.checkStopNurse] || ''}}</td>
        </tr>
        </tbody>
      </table>
    </div>
              ...
              for (let x = 0; x < list.length; x++) {
                let _d = list[x]
                  if(!data[_d['medicalAdviceId']]){
                    key_arr.push(_d['medicalAdviceId'])
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          for (let x = 0; x < list[i].medicalAdviceItemDoList.length; x++) {
            ...
            if (x === 0) {
              ...
              console.log(nItem['medicalItemName'])
            } else {
