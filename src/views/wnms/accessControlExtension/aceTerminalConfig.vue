 ...
 * @LastEditTime: 2022-06-14 11:34:48
 * @LastEditors: 范博
 * @Description: 
 * @FilePath: \yarward-wnms-frontEnd\src\views\wnms\accessControlExtension\aceTerminalConfig.vue
-->
<template>
      ...
      <el-form ref="configContent" :model="configContent" :rules="rules" label-width="160px">
        ...
        <el-form-item label="测温范围">
          <template>
            <div style="display:grid;grid-template-columns:70% 30%">
              ...
              <div>
                测温补偿：<el-input style="width:80px" v-model="configContent.testTeptCompensate"></el-input>摄氏度
              </div>
            </div>
          </template>
        </el-form-item>
        ...
        <el-form-item label="开门方式">
          <el-checkbox-group v-model="configContent.doorOpenType">
            ...
            <el-input v-model.trim="configContent.doorGuardPasswordGuard" type="text" maxlength="7" class="w100" />
          </el-checkbox-group>
        </el-form-item>
        ...
        <el-form-item label="开启时间" v-if="configContent.mulAcceptRequestsDevices.length !== 0">
          <el-button round type="primary" size="mini" @click="onAddFirstunlockDutyRoomOpenTime" v-if="configContent.unlockDutyRoomOpenTime.length === 0">添加</el-button>
          <section v-else>
            ...
            <el-col v-for="(o, index) in 4" :key="o" :xs="12" :sm="6" :md="5" :lg="5" :xl="4" style="margin-bottom: 10px">
              <el-card :body-style="{ padding: '0px' }" shadow="never" style="padding: 10px">
                <div style="display:grid;grid-template-columns:20% 80%;align-items: center;">
                  <div>{{configContent.volumeGuard[index].configLabel}}</div>
                  <div>
                    <el-time-picker v-model="configContent.volumeGuard[index].config.Time" format="HH:mm:ss" value-format="HH:mm:ss" :picker-options="{
                    selectableRange: '00:00:00 - 23:59:59',
                  }" :clearable="false" placeholder="选择时间" style="width: 100%" />
                  </div>
                </div>
                <div style="display:grid;grid-template-columns:35% 65%;align-items: center;">
                  <div>{{configContent.volumeGuard[index].configLabel}}音量</div>
                  <div>
                    <el-select class="volume-select" v-model="configContent.volumeGuard[index].config.Vol" placeholder="请选择音量" style="width: 100%; margin-top: 10px">
        ...
        <el-form-item label="终端亮度">
          <el-row :gutter="10">
            <el-col v-for="(o, index) in 4" :key="o" :xs="12" :sm="6" :md="5" :lg="5" :xl="4" style="margin-bottom: 10px">
              <el-card :body-style="{ padding: '0px' }" shadow="never" style="padding: 10px">
                <div style="display:grid;grid-template-columns:20% 80%;align-items: center;">
                  <div>{{configContent.lightGuard[index].configLabel}}</div>
                  <div>
                    <el-time-picker v-model="configContent.lightGuard[index].config.Time" format="HH:mm:ss" value-format="HH:mm:ss" :picker-options="{
                    selectableRange: '00:00:00 - 23:59:59',
                  }" :clearable="false" placeholder="选择时间" style="width: 100%" />
                  </div>
                </div>
                <div style="display:grid;grid-template-columns:35% 65%;align-items: center;">
                  <div>{{configContent.lightGuard[index].configLabel}}亮度</div>
                  <div>
                    <el-select class="light-select" v-model="configContent.lightGuard[index].config.Light" placeholder="请选择亮度" style="width: 100%; margin-top: 10px">
...
<script>
import configMixin from "./mixin/mixin.js";
...
export default {
  ...
  data () {
    return {
      ...
      configContentJSON: '{ "WegingCode": 11, "doorOpenType": [], "controlType": "", "controlTypeCardNumber": "", "cardTransmissionOrder": "", "cardTransmissionHex": "", "autoGoIndex": false, "autoGoIndexTime": 15, "doorGuardPasswordGuard": "3585668", "normalOpenTimeGuard": [], "autoCloseDoorTimeGuard": 5, "mulAcceptRequestsDevices": [], "unlockDutyRoomOpenTime": [{ start: "00:00:00", end: "23:59:59" }], "volumeGuard": [ { "Vol": "10", "Time": "08:00:00" }, { "Vol": "8", "Time": "12:00:00" }, { "Vol": "10", "Time": "14:00:00" }, { "Vol": "6", "Time": "18:00:00" } ], "lightGuard": [ { "Light": "50", "Time": "08:00:00" }, { "Light": "50", "Time": "12:00:00" }, { "Light": "50", "Time": "14:00:00" }, { "Light": "50", "Time": "18:00:00" } ], "autoRestarGuard": { "time": "08:00:00", "state": false }, "deviceSettingGuard": false }',
      // 配置内容
      testTeptRangeMarks: {
        35: '35°C',
        // 37.5: '37°C',
      ...
      configContent: {
        homePageTips: '',//首页提示语
        isTestTept: false,//人脸识别时测温
        testTeptRange: [37.5, 41],//测温范围
        testTeptCompensate: '',//测温补偿
  ...
  methods: {
    sChange () {
      console.log("---->" + this.configContent.testTeptRange);
    },
    sInput () {
      console.log("---->" + this.configContent.testTeptRange);
    },
...
/* 选择科室名称下的颜色选择器不隐藏色板 */
.dept-popper .el-color-dropdown__main-wrapper,
.dept-popper .el-color-alpha-slider {
  display: block;
}
...
.volume-select:before {
  content: "";
  background: url("../../../assets/wnms/img/icons_volume.png") center / contain no-repeat;
}
