<template>
  <div class="typt-list-container statistics-body ">
    <!--查询条件模块 === START-->
    <el-form :model="search" :rules="rules" ref="search" class="statistics-search" :inline="true">
...
export default {
  ...
  data() {
    ...
    return {
      emptyDeptIds: false, // 判断是否让科室的input框变成红色
      search: {
        startTimeDate: "",
        endTimeDate: "",
        startTimePoint: "",
      ...
      rules: {
        checkFormKeshi: [{ validator: checkKeshi, trigger: "blur" }],
    ...
    queryHandler(type) {
      ...
      this.$refs.search.validate((valid) => {
        if (!valid) {
          ...
          statisticsApi.getKeshiData(search).then((res) => {
            that.x = res.data.x;
            that.average = res.data.average;
