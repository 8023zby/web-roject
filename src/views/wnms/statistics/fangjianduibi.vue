<template>
  <div class="typt-list-container statistics-body ">
    <!--查询条件模块 === START-->
    <el-form ref="search" :rules="rules" class="statistics-search" :inline="true" :model="search">
  ...
  data() {
    var checkKeshi = (rule, value, callback) => {
      if (!this.search.deptId) {
        ...
        return callback(new Error("请选择一个科室"));
      }
    ...
    queryHandler(type) {
      ...
      this.$refs.search.validate((valid) => {
        if (!valid) {
          ...
          statisticsApi.getFangjianData(search).then((res) => {
            that.x = res.data.x;
