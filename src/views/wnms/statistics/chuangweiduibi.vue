  ...
  <div class="typt-list-container statistics-body ">
    ...
    <el-form :rules="rules" ref="search" class="statistics-search" :inline="true" :model="search">
      ...
      <el-form-item label="日期:" prop="checkFormDate">
        <el-date-picker v-model="search.startTimeDate" value-format="yyyy-MM-dd" format="yyyy-MM-dd" :picker-options="pickerOptions" placeholder="请选择" />
    ...
    queryHandler(type) {
      ...
      this.$refs.search.validate((valid) => {
        if (!valid) {
          ...
          statisticsApi.getChuangWeiData(search).then((res) => {
            that.x = res.data.x;
