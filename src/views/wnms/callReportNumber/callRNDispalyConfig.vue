  ...
  <div class="typt-list-container">
    ...
    <el-tabs @tab-click="menuChange()" class="myTabs" v-model="activeName" :tab-position="tabPosition">
      <template v-for="menu in callRNMenu">
        <el-tab-pane :label="menu.value" :name="menu.key">
        </el-tab-pane>
      </template>
    </el-tabs>
          ...
          <div class="form-content">
            <template v-for="field in item.config">
              <template v-if="field.type==='switch'">
                <el-form-item :label="`${field.label}：`" :label-width="`${field.labelWidth}`" :style="`display:${field.display}`">
...
import { AddRNConfigInfo, AddRNDisplayConfigInfo, QueryRNConfig, QueryRNShowConfig } from '@/api/wnms/callReportNumber/callRNField.js';
import { getDeptsList } from '@/api/wnms/msgManage/patientMsg';

export default {
  name: "callRNDisplayConfig",
  ...
  data () {
    return {
      ...
      callRNMenu: [
        ...
        {
          key: "dutyRoom", value: "值班室呼叫",
        },
        {
          key: "inpatient", value: "病区门禁呼叫",
        },
        {
          key: "host", value: "主机呼叫",
        },
        {
          key: "visit", value: "探视呼叫",
        },
        {
          key: "mobileDevice", value: "移动通讯终端呼叫",
        },
        {
          key: "infusionDevice", value: "输液夹断器呼叫",
        }
      ],
    };
  },
  ...
  methods: {
      ...
      getDeptsList().then(res => {
        if (res.status == 200) {
          this.$nextTick(() => {
            ...
            this.checkDepts = this.deptsList[0].deptId;
            this.queryConfig();
          });
        }
      }).catch(err => {
    ...
    initFormFields () {
      this.callRNMenu.forEach(item => {
        ...
        let obj = [
          {
            label: '显示房间号',
            fieldName: "displayRoomNo",
            type: 'switch',
            labelWidth: '120px',
            val: '0',
            display: this.isDisplay('displayRoomNo', item.key)
          },
          {
            label: '房间号后缀',
          ...
          {
            label: '未处理显示方式',
            fieldName: "UnTDisplayType",
            type: 'radio',
            labelWidth: '140px',
            val: '1',
            display: this.isDisplay('UnTDisplayType', item.key),
            items: [
              {
                key: '1', value: '显示未处理'
              },
    ...
    isDisplay (field, type) {
      ...
      switch (type) {
        ...
        case "wc":
        case "door":
          if (field == 'displayRoomNo')
            isDisplay = 'none';
          else if (field == 'nurseLevVoice')
            isDisplay = 'none';
          break;
        case "dutyRoom":
        case "inpatient":
    ...
    queryConfig () {
      QueryRNShowConfig(this.checkDepts.toString()).then(res => {
        if (res.status == 200) {
          if (res.data) {
