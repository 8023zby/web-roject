    ...
    <div class="msg_content">
      <ul class="infinite-list" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" style="overflow:auto">
        <li v-for="(item, index) in this.pagination.list" :key="index" :class="{active:index == current}">
          ...
          <div class="msg-type">{{ item.msgType }}
          ...
          <div class="content">
            <div v-if="item.viewType == 2" class="content_bg flex">
              <m-audio :ref="'maudio' + index" :astop="all_stop" :duration="item.secondsDuration" @click.native="show(item, index)" :src="ListConvert(item)"></m-audio>
            </div>
            <span @click="show(item, index)" v-else class="content_bg flex" v-html="item.content"></span>
          </div>
        </li>
...
export default {
  ...
  methods: {
    ...
    getNoRead (isRead) {
      ...
      searchs.deptId = selectDeptInfo();
      searchs.referenceType = "USER";
      searchs.referenceId = this.search.referenceId;
      searchs.onlyFlag = "1,2,3,4,5,6,7,8,9,0,14";
    ...
    loadMore () {
      ...
      params.onlyFlag = "1,2,3,4,5,6,7,8,9,0,14";
