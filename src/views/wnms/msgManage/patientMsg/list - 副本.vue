  ...
  <div class="main_patient">
    <div class="msg_title">
      <div class="right_btn">
        ...
        <el-button type="primary" v-if="lookAll" @click="allMsg(true)">查看全部</el-button>
        <el-button
          v-else
          type="primary"
          @click="allMsg(false)"
        >仅看未读:({{ length > 99 ? '99+' : length }})</el-button>
        <el-button type="primary" @click="allConfirm">全部标记为已读</el-button>
      </div>
    </div>
    <!--滑动区域-->
    <div class="msg_content">
      ...
      >
        <li
          v-for="(item, index) in   this.pagination.list"
          :key="index"
        ...
        >
          ...
          <div class="title">
            <span v-if="item.isRead===0?true:false">有新消息!</span>
          </div>
          <div class="content">
            <div v-if="item.viewType == 2" class="content_bg flex">
              <m-audio
                :ref="'maudio' + index"
                :astop="all_stop"
                :duration="item.secondsDuration"
                @click.native="show(item, index)"
                :src="ListConvert(item)"
              ></m-audio>
            </div>

            <span @click="show(item, index)" v-else class="content_bg flex" v-html="formant(item)"></span>
          </div>
        </li>
      </ul>
      <p v-if="loading">加载中...</p>
      <p v-if="noMore" class="nomore">没有更多了</p>
      <div class="nomsg" v-if="this.pagination.list.length <=0">暂无数据.</div>
    </div>
    ...
    <el-dialog title :visible.sync="dialogPlay" width="50%" @close="closeDialog">
      ...
      <m-video ref="refvideo" class="video_bottom"></m-video>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    return {
      lookAll: false, //false 未读  true 全部
      busy: false,
      loading: false, // 加载数据
  ...
  methods: {
    ...
    initQueryCriteria (form = {}) {
      return deepMerge(form, {
        ...
        //  onlyFlag: '1',// this.onlyFlag,
        //referenceId: getUser()
        referenceType: 'WEB',
        'aaaa': 1,
        size: 1,// this.page.size,
        page: 1, //this.page.pageindex
    ...
    show (item, index) {
      // 文字 如果是自动提醒 则
      if (item.viewType === "0") {
        if (item.msgResource === "AUTO-MSG" && (item.url != null || item.url !== "")) {
    ...
    allConfirm () {
      this.$confirm('确定要进行全部标记为已读操作吗?', '全部标记为已读', {
        ...
        // 设置已读后台还未添加状态
        api.markRead(this.serach).then((res) => {
          if (this.stateDetect(res)) {
            this.allMsg(1)
            this.getNoRead(0)
          }
    ...
    loadMore (isRead = 0) {
      ...
      //   referenceType: 'WEB',
      // }
      api.getInfo(self.createQueryParams()).then((res) => {
