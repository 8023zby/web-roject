      ...
      <el-form ref="configContent" :model="configContent" :rules="rules" label-width="160px">
        ...
        <div class="el-form-item-left">
          ...
          <el-form-item label="开启隐私保护" prop="name">
            <el-switch
              v-model="configContent.isFilterName"
        ...
        <div>
          <el-form-item label="定时启用息屏时间">
            <el-switch
              v-model="configContent.closeScreenTime.closeScreenOpen"
              :active-color="switchActiveColor"
              active-value="1"
              inactive-value="0"
            />
          ...
          <el-form-item>
            <el-time-picker
              v-model="configContent.closeScreenTime.startTime"
              format="HH:mm:ss"
              value-format="HH:mm:ss"
              placeholder="选择开始时间"
              :clearable="false"
              :disabled="configContent.closeScreenTime.closeScreenOpen === '0'"
            />-
            <el-time-picker
              v-model="configContent.closeScreenTime.stopTime"
              format="HH:mm:ss"
              value-format="HH:mm:ss"
              placeholder="选择结束时间"
              :clearable="false"
          ...
          <el-form-item prop="brightViewSeconds">
            点亮屏幕
            <el-input
              v-model.trim="configContent.brightViewSeconds"
              type="text"
              maxlength="3"
              class="w60"
            />秒后自动息屏
        ...
        <div>
          <el-form-item label="呼叫需要验证">
            <el-switch v-model="configContent.passwordOpen" active-value="1" inactive-value="0"/>
          </el-form-item>
          <el-form-item prop="password" :rules="[
              {
                required: configContent.passwordOpen === '1',
                message: '不能为空',
                trigger: 'blur',
              },
            ...
            验证密码
            <el-input v-model="configContent.password" type="number" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)))" :disabled="configContent.passwordOpen !== '1'" class="w150" />
          </el-form-item>
        </div>

        <el-form-item label="储存消息条是否显示" prop="isHandlingCalls">
          <el-switch
            v-model="configContent.isHandlingCalls"
            :active-color="switchActiveColor"
            active-value="1"
            inactive-value="0"
          />
        ...
        <div>
          <el-form-item label="语音报号">
            <el-switch
              v-model="configContent.voice.voiceOpen"
              :active-color="switchActiveColor"
              active-value="1"
              inactive-value="0"
            />
          ...
          <el-form-item>
            <el-time-picker
              v-model="configContent.voice.startTime"
              format="HH:mm:ss"
              value-format="HH:mm:ss"
              placeholder="选择开始时间"
              :clearable="false"
              :disabled="configContent.voice.voiceOpen === '0'"
            />-
            <el-time-picker
              v-model="configContent.voice.stopTime"
              format="HH:mm:ss"
              value-format="HH:mm:ss"
              placeholder="选择结束时间"
              :clearable="false"
        ...
        <div>
          <el-form-item label="按键功能">
          </el-form-item>
          <div style="padding-left: calc(4em + 12px)">
            <el-form-item label="短按功能">
              <el-select v-model="configContent.keyPress.keySPress1" placeholder="请选择">
                <el-option label="关闭" value="0"></el-option>
                <el-option label="默认" value="1"></el-option>
                <el-option label="护理" value="2"></el-option>
                <el-option label="呼叫" value="3"></el-option>
                <el-option label="增援" value="4"></el-option>
                <el-option label="清除所有未处理" value="5"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="长按功能">
              <el-select v-model="configContent.keyPress.keyLPress1" placeholder="请选择">
                <el-option label="关闭" value="0"></el-option>
                <el-option label="默认" value="1"></el-option>
                <el-option label="护理" value="2"></el-option>
                <el-option label="呼叫" value="3"></el-option>
        ...
        <el-form-item label="终端亮度">
          <el-row :gutter="10">
            ...
            >
              <el-card :body-style="{ padding: '0px' }" shadow="never" style="padding: 10px">
                <el-time-picker
                  v-model="configContent.light[index].Time"
                  format="HH:mm:ss"
                  value-format="HH:mm:ss"
                  :picker-options="{
                    selectableRange: '00:00:00 - 23:59:59',
                ...
                <el-select
                  class="light-select"
                  v-model="configContent.light[index].Light"
                  placeholder="请选择亮度"
                  style="width: 100%; margin-top: 10px"
                >
                  <el-option
...
import { getTemplateByDeptId, getTemplateByDeviceId } from '../../../api/wnms/msgManage/systemConfig'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      // 配置页面名称
      deviceConfigName: "iDoorWayExtension",
      // 配置内容
      configContent: JSON.parse(JSON.stringify(defaultConfig)),
  ...
  methods: {
      ...
      try {
        ...
        if (res.data) {
          Object.keys(this.configContent).map(key => {
            this.configContent[key] = res.data[key] || this.defaultConfig[key]
          })
        }
      } catch (error) {}
    },
      ...
      try {
        ...
        if(typeof res.data === 'string') {
          switch (res.data.toUpperCase()) {
            ...
            case 'MATERIAL':
            case 'UPDATEPACKAGE':
              this.$message.warning(`正在发送${ESetTemplateType[res.data.toUpperCase()]},请稍后重试`)
              return
              break;
          }
    ...
    goParent () {
      let path = ''
      if (this.deptId) {
        path = `/deveiceA10IXL/iDoorWayExtension?deptId=${this.deptId}`
      } else {
        path = `/deveiceA10IXL/iDoorWayExtension`
      }
      this.$router.push({
        path: path
      })
    ...
    async setMaterialDept() {
      if(Object.prototype.toString.call(this.selectedTemplate) !== '[object Object]' || JSON.stringify(this.selectedTemplate) === '{}') return this.$message.error('请先选择模板')
      try {
        let res = await API.setMaterialDept({
          deptId: this.nurseUnitId,
          deviceIds: this.deviceId,
          deviceModel: this.selectedTemplate.deviceModel,
          deviceType: this.deviceConfigName
        })
        if(typeof res.data === 'string') {
          switch (res.data.toUpperCase()) {
