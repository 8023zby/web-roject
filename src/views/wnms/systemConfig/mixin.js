...
export default {
  ...
  methods: {
     ...
     */
    getData (deviceType) {
      console.log("deviceGetData--->" + deviceType);
      // alert(deviceType)
        ...
        if (response.status === 200 && response.data !== null) {
          switch (deviceType) {
            ...
            case 'other':
              ...
              // console.log(this.medicalMainframeList);
              if (!this.hasDeviceId(this.medicalMainframeList, _data)) {
                _data.msgForwardDeviceId = '';
              }
            ...
            case 'doorway':
              ...
              this.configContent.ReturnHomePage = !this.configContent.ReturnHomePage ? '' : '' + this.configContent.ReturnHomePage;
    ...
    hasDeviceId (medicalMainframeList, configContent) {
      let b = false;
      for (let i = 0; i < medicalMainframeList.length; i++) {
        let item = medicalMainframeList[i];
        if (item.deviceId == configContent.msgForwardDeviceId) {
          console.log(item.deviceId + "__" + configContent.msgForwardDeviceId);
          b = true;
          break;
        }
      };
      ...
      this.$refs['configContent'].validate((valid) => {
        if (valid) {
          ...
          if (deviceConfigName === 'doorway') {
            // 门口机数据处理
            if(this.configContent.ReturnHomePage !== '' && Number(this.configContent.ReturnHomePage) < 5){
               this.$message({
                message: '返回主页时间不能小于5秒',
                type: 'warning'
              });
              return
            }
            this.configContent.ReturnHomePage = Number(this.configContent.ReturnHomePage);
          }
          ...
          if (deviceConfigName == 'accessControl') {
            ...
            if (this.configContent.homePageTips.includes('\n')) {
              this.$message({
                message: '首页欢迎语不能包含换行符',
                type: 'warning'
              });
              return;
            }
            if (this.configContent.testTeptCompensate < -10 || this.configContent.testTeptCompensate > 10) {
              this.$message({
                message: '测温补偿范围在-10~10之间',
                type: 'warning'
              });
              return;
            }
            if (this.configContent.doorOpenType.includes('pwd')) {
              if (this.configContent.doorGuardPasswordGuard == '') {
                this.$message({
                  message: '请输入门禁密码',
                  type: 'warning'
                });
          ...
          if (deviceConfigName === 'other') {
            ...
            const unlockDutyRoomOpenTime = JSON.parse(JSON.stringify(this.configContent.unlockDutyRoomOpenTime));
            let isCoincide = false; // 是否有重合时间
            if (!unlockDutyRoomOpenTime.every(v => (v.start && v.end))) return this.$message.error('请补充完整多终端接受门禁请求开启时间');
            while (unlockDutyRoomOpenTime && unlockDutyRoomOpenTime.length) {
              let top = unlockDutyRoomOpenTime.shift();
              isCoincide = !unlockDutyRoomOpenTime.every(v => (top.start > v.end || top.end < v.start));
              if (isCoincide) break;
            }
            if (isCoincide) return this.$message.error('多终端接受门禁请求开启时间有重合时间，请修改后重新提交');
          }
          ...
          addConfig.nurseUnitId = this.nurseUnitId;
