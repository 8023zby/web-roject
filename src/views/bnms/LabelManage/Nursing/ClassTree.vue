<template>
  <div style="position: relative; height: 100%; background-color: #fff">
    ...
    <div class="nrms_nurse_list">
      ...
      <div class="list">
        <div class="btns">
          <el-button type="primary" @click="nodeAdd(null)" v-uni="'biaoqianfenlei_add'" v-checkAuth="'addClass'">添加</el-button>
          <el-button type="primary" @click="nodeSort" v-uni="'biaoqianfenlei_sort'" v-checkAuth="'sortClass'">排序</el-button>
        </div>
        <el-tree
          ref="classTree"
          :data="classTreeData"
        ...
        >
          <div class="custom-tree-node" slot-scope="{ node, data }">
            ...
            <div class="comp-tr-node--name" :title="node.label">{{ node.label }}</div>
            <!-- 按钮 -->
            <div v-if="data.classId && checkAuth('editClass,deleteClass')">
              <!-- 编辑 -->
              <span style="margin:0 10px 0 0" v-uni:[nodeKey]="'biaoqianfenlei_edit'" :key="'edit' + nodeKey" @click.stop="nodeEdit(node, data)" v-checkAuth="'editClass'">
                    <i class="el-icon-edit-outline"></i>
                  </span>
              <!-- 删除 -->
              <span @click.stop="nodeDel(node, data)"  v-uni:[nodeKey]="'biaoqianfenlei_delete'" :key="'delete' + nodeKey" v-checkAuth="'deleteClass'">
    ...
    <el-dialog
      ...
      width="780px"
    >
      <class-sort ref="classSort" @sortCallBack="sortCallBack"></class-sort>
      <div slot="footer">
        <el-button v-uni="'biaoqianfenleisort_cacel'" type="primary" @click="sortDialogClose">关闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
    ...
    nodeAdd (data) {
      ...
      this.$prompt('分类名称:', title, opts).then(({ value }) => {
        ...
        ApiClassifyManage.save(_data)
          .then(() => {
            this.getData()
            this.$refs.classSort && this.$refs.classSort.getClassifyData()
            this.$emit('refresh')
