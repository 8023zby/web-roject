<template>
  <layout-aside v-loading="loading">
    ...
    <div slot="main">
      <layout-list>
        ...
        <div slot="right_btn" class="tool-bar">
          <el-button type="primary" size="small" @click="evaluate" :disabled="btn_disable"
          >评价
          </el-button>
          <!--<el-button type="primary" v-print="'#myHtmlElement'" size="small"  :disabled="btn_disable" style="background:#13ce67 !important;border-color: #13ce67 !important;"-->
          <!--&gt;打印-->
          <!--</el-button>-->
          <el-button type="primary"
                     @click="printing()"
                     size="small" style="background:#13ce67 !important;border-color: #13ce67 !important;"
          >打印
          </el-button>
        </div>
        <div slot="main" class="main-list">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-button @click="getDetails(scope.row)" type="text"
                >查看
                </el-button>
              </template>
            </el-table-column>
            <el-table-column label="操作" min-width="160" align="center">
              <template slot-scope="scope">
                <el-button @click="cancelEducation(scope.row)" type="text"  v-if="scope.row.state === 1 && scope.row.isEnable === 1" :disabled="btn_disable">
                  撤回
                </el-button>
                ...
                <el-button @click="eduView(scope.row)" type="text"
                >预览
                </el-button>
                <el-button  type="text" :disabled="scope.row.state === 1 && scope.row.isEnable === 1" v-if="scope.row.state === 1 && scope.row.isEnable === 1"
                > <span style="color:#d1d2d3">编辑</span>
                </el-button>
                <el-button @click="editEducation(scope.row)" type="text" :disabled="btn_disable" v-if="scope.row.state === 0 || (scope.row.state === 1 && scope.row.isEnable === 0)"
                >编辑
                </el-button>
                <el-button
                        type="text"
                ...
                > <span style="color:#d1d2d3">删除</span>
                </el-button>
                <el-button
                        type="text"
                        style="color:#F56C6C;"
    ...
    <el-dialog title="宣教详情" :visible.sync="dialogTableVisible">
      <el-table :data="detailsData"  style="width: 100%;overflow: auto;max-height: 400px">
        <el-table-column property="" label="序号"  type="index"></el-table-column>
        <el-table-column property="" label="宣教打卡">
          <el-table-column property="medicalCareName" label="打卡人">
          </el-table-column>
          <el-table-column property="beginTime" label="打卡时间">
          </el-table-column>
        </el-table-column>
        <el-table-column property="" label="患者阅读">
          <el-table-column property="viewTotalSeconds" label="阅读时间（分钟）">
          </el-table-column>
          <el-table-column property="signItUrl" label="签字状态">
            <template slot-scope="scope">
          ...
          <el-table-column property="endTime" label="签字时间">
          </el-table-column>
        </el-table-column>
      </el-table>
    </el-dialog>
    <div style="display: none">
      <div id="previewElement">
        <div style="border:1px solid #DCDFE6;padding: 10px;margin-top: 20px;min-height: 100px;">
          ...
          <div v-html="articleDetails.contentText" class="contentText"></div>
        </div>
    ...
    <el-dialog title="预览健康宣教" :visible.sync="articleVisible" >
      <div style="overflow: auto;height: 600px">
        <div style="text-align: right">
          <el-button type="primary" size="small" @click="printView()"
          >打印
          </el-button>
          <!--<el-button type="primary" size="small" @click="printView()" :disabled="btn_disable"-->
          <!--&gt;打印-->
          <!--</el-button>-->
        </div>
        <div style="border:1px solid #DCDFE6;padding: 10px;margin-top: 20px;min-height: 100px;overflow: auto">
          <div style="text-align: center;font-weight: bold;font-size: 20px;">{{articleDetails.title}}</div>
          <div v-html="articleDetails.contentText"></div>
        </div>
        <div style="padding: 10px;background: #E0EFFF;display: flex;justify-content: space-around">
          <div>
            <label>患者签字&nbsp;&nbsp;&nbsp;</label>
...
<script>
  ...
  import { addEduRecord, getEduRecord, recordDetails, delEduRecord, editEduRecord, recordState, addEduComment, getEdArticleDetails } from '@/api/hpms/eduCation/situation'
  export default {
        ...
        getEdArticleDetails(row.eduId)
                .then((res) => {
                  if(res.status === 200){
                    ...
                    this.articleDetails.contentText = res.data.contentText
                    recordState({
                      articleId: row.eduId,
                      patientId: this.search_from.patientId
                    }).then(res => {
                      if(res.data !==  null && res.data.length > 0){
                        for(let i of res.data){
                          if(i.signItUrl !== undefined &&  i.signItUrl !== null && i.signItUrl !== ''){
    ...
    components: {
      PatientList, layoutAside, layoutList, PrintTemplate
    }
  }
</script>
